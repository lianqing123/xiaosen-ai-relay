...
type APIKey struct {
	...
	// Preferred active subscription group used to bill route keys such as Claude
...
func (*APIKey) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case apikey.FieldID, apikey.FieldUserID, apikey.FieldGroupID, apikey.FieldPreferredSubscriptionGroupID:
...
func (_m *APIKey) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case apikey.FieldPreferredSubscriptionGroupID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field preferred_subscription_group_id", values[i])
			} else if value.Valid {
				_m.PreferredSubscriptionGroupID = new(int64)
				*_m.PreferredSubscriptionGroupID = value.Int64
			}
...
func (_m *APIKey) String() string {
	...
	builder.WriteString(", ")
	if v := _m.PreferredSubscriptionGroupID; v != nil {
		builder.WriteString("preferred_subscription_group_id=")
		builder.WriteString(fmt.Sprintf("%v", *v))
	}
	builder.WriteString(", ")
