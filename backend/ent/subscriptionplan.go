...
type SubscriptionPlan struct {
	...
	// 套餐额外包含的订阅分组 ID；group_id 始终作为主分组自动包含
...
func (*SubscriptionPlan) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case subscriptionplan.FieldIncludedGroupIds:
			values[i] = new([]byte)
	...
	for i := range columns {
		...
		case subscriptionplan.FieldIncludedGroupIds:
			if value, ok := values[i].(*[]byte); !ok {
				...
				if err := json.Unmarshal(*value, &_m.IncludedGroupIds); err != nil {
...
func (_m *SubscriptionPlan) String() string {
	...
	builder.WriteString(fmt.Sprintf("%v", _m.IncludedGroupIds))
	builder.WriteString(", ")
