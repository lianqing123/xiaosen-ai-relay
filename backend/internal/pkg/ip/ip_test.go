...
func TestGetClientIPPrefersForwardedRealIP(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	tests := []struct {
		...
		{
			name: "xff skips private address with port",
			headers: map[string]string{
				"X-Forwarded-For": "10.0.0.9:1234, 9.9.9.9",
			},
			expected: "9.9.9.9",
		},
		{
			name: "xff falls back first private address when all private",
			headers: map[string]string{
				"X-Forwarded-For": "10.0.0.9:1234, 192.168.1.2",
			},
			expected: "10.0.0.9",
		},
	}
