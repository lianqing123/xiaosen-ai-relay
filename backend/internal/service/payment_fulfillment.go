...
func (s *PaymentService) doSub(ctx context.Context, o *dbent.PaymentOrder) error {
	...
	if !g.IsSubscriptionType() {
		return fmt.Errorf("group %d is not a subscription type", gid)
	}
	...
	if err := s.ensureSubscriptionAPIKeyAccess(ctx, o.UserID, gid); err != nil {
		slog.Warn("subscription api key access sync failed", "orderID", o.ID, "userID", o.UserID, "groupID", gid, "error", err)
		s.writeAuditLog(ctx, o.ID, "SUBSCRIPTION_API_KEY_SYNC_FAILED", "system", map[string]any{
			"userID":  o.UserID,
			"groupID": gid,
			"error":   err.Error(),
		})
...
func (s *PaymentService) ensureSubscriptionAPIKeyAccess(ctx context.Context, userID, subscriptionGroupID int64) error {
	if s == nil || s.entClient == nil || userID <= 0 || subscriptionGroupID <= 0 {
		return nil
	}
	...
	keys, err := s.entClient.APIKey.Query().
		Where(
			...
			apikey.StatusEQ(StatusActive),
		).
		WithGroup().
		All(ctx)
	if err != nil {
		return fmt.Errorf("list active api keys: %w", err)
	}
	...
	for _, key := range keys {
		...
		if key.GroupID != nil && *key.GroupID == subscriptionGroupID {
			continue
		}
		if key.Edges.Group != nil && key.Edges.Group.SubscriptionType == SubscriptionTypeSubscription {
			continue
		}

		if err := s.entClient.APIKey.UpdateOneID(key.ID).
			Where(apikey.DeletedAtIsNil()).
			SetGroupID(subscriptionGroupID).
			Exec(ctx); err != nil {
			return fmt.Errorf("bind api key %d to subscription group %d: %w", key.ID, subscriptionGroupID, err)
		}
...
func (s *PaymentService) applyAffiliateRebateForOrder(ctx context.Context, o *dbent.PaymentOrder) error {
	baseAmount, ok := affiliateRebateBaseAmountForOrder(o)
	if !ok {
		return nil
	}
...
func affiliateRebateBaseAmountForOrder(o *dbent.PaymentOrder) (float64, bool) {
	if o == nil {
		return 0, false
	}
	switch o.OrderType {
	case payment.OrderTypeBalance:
		baseAmount := paymentAmountBeforeFee(o.PayAmount, o.FeeRate)
		if baseAmount <= 0 {
			baseAmount = o.Amount
		}
		if baseAmount <= 0 {
		...
		return roundTo(baseAmount, 8), true
...
func paymentAmountBeforeFee(payAmount, feeRate float64) float64 {
	...
	return payAmount / (1 + feeRate/100)
}
