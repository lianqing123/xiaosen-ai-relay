...
func (s *accountRepoStubForAdminList) ListWithFilters(_ context.Context, params pagination.PaginationParams, platform, accountType, status, search string, groupID int64, privacyMode string, source string) ([]Account, *pagination.PaginationResult, error) {
...
func TestAdminService_ListAccounts_WithSource(t *testing.T) {
	t.Run("source 参数正常传递到 repository 层", func(t *testing.T) {
		repo := &accountRepoStubForAdminList{
			listWithFiltersAccounts: []Account{{ID: 3, Name: "Kiro Gateway"}},
			listWithFiltersResult:   &pagination.PaginationResult{Total: 1},
		}
		svc := &adminServiceImpl{accountRepo: repo}

		accounts, total, err := svc.ListAccounts(context.Background(), 1, 20, PlatformAnthropic, AccountTypeAPIKey, StatusActive, "", 0, "", AccountSourceKiroGateway, "name", "asc")
		require.NoError(t, err)
		require.Equal(t, int64(1), total)
		require.Equal(t, []Account{{ID: 3, Name: "Kiro Gateway"}}, accounts)
		require.Equal(t, AccountSourceKiroGateway, repo.listWithFiltersSource)
	})
}
