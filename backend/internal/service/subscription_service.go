...
// ResolveIncludedRouteBillingSubscription maps an add-on route subscription
// (for example Kiro) back to the user's primary package subscription when that
// route is included by a subscription plan. Routing still uses the API key
// group; quota checks and usage increments use the returned subscription.
func (s *SubscriptionService) ResolveIncludedRouteBillingSubscription(ctx context.Context, userID int64, routeGroup *Group, routeSubscription *UserSubscription, preferredGroupID *int64) (*UserSubscription, *Group, error) {
	if routeGroup == nil || !routeGroup.IsSubscriptionType() {
		return routeSubscription, SubscriptionBillingGroup(routeGroup, routeSubscription), nil
	}
	if s == nil || s.entClient == nil || s.userSubRepo == nil {
		return routeSubscription, SubscriptionBillingGroup(routeGroup, routeSubscription), nil
	}

	primaryGroupIDs, err := s.includedRoutePrimaryGroupIDs(ctx, routeGroup.ID)
	if err != nil {
		return nil, nil, err
	}
	...
	if preferredGroupID != nil && *preferredGroupID > 0 && !(routeGroup.IsClaudeRoute() && *preferredGroupID == routeGroup.ID) {
		if preferred := resolvePreferredIncludedRouteSubscription(routeGroup, routeSubscription, subs, primaryGroupIDs, preferredGroupID); preferred != nil {
			return preferred, SubscriptionBillingGroup(routeGroup, preferred), nil
		}
		return nil, nil, ErrSubscriptionNotFound
	}

	var best *UserSubscription
	if routeSubscription != nil && !routeGroup.IsClaudeRoute() && routeSubscription.Group != nil && routeSubscription.Group.IsActive() && routeSubscription.Group.IsSubscriptionType() {
		cp := *routeSubscription
		best = &cp
	}
	for i := range subs {
		sub := subs[i]
		if !primaryGroupIDs[sub.GroupID] {
			continue
		}
		if sub.Group == nil || !sub.Group.IsActive() || !sub.Group.IsSubscriptionType() {
			continue
		}
		if betterIncludedRouteBillingSubscription(&sub, best) {
			cp := sub
			best = &cp
		}
	}
	...
	return best, SubscriptionBillingGroup(routeGroup, best), nil
}

func resolvePreferredIncludedRouteSubscription(routeGroup *Group, routeSubscription *UserSubscription, subs []UserSubscription, primaryGroupIDs map[int64]bool, preferredGroupID *int64) *UserSubscription {
	if preferredGroupID == nil || *preferredGroupID <= 0 {
		return nil
	}
	if routeGroup != nil && routeSubscription != nil && routeSubscription.GroupID == *preferredGroupID {
		if routeGroup.IsClaudeRoute() {
			return nil
		}
		cp := *routeSubscription
		if cp.Group == nil {
	...
	if !primaryGroupIDs[*preferredGroupID] {
		return nil
	}
	for i := range subs {
		sub := subs[i]
		if sub.GroupID != *preferredGroupID {
			continue
		}
		if sub.Group == nil || !sub.Group.IsActive() || !sub.Group.IsSubscriptionType() {
			continue
...
func (s *SubscriptionService) includedRoutePrimaryGroupIDs(ctx context.Context, routeGroupID int64) (map[int64]bool, error) {
	plans, err := s.entClient.SubscriptionPlan.Query().All(ctx)
	if err != nil {
		return nil, fmt.Errorf("load included route plans: %w", err)
	}
	out := make(map[int64]bool)
	for _, plan := range plans {
		if plan == nil || plan.GroupID == routeGroupID {
			continue
		}
		if !planIncludesGroup(plan.IncludedGroupIds, routeGroupID) {
			continue
		}
		out[plan.GroupID] = true
	}
	return out, nil
}

func planIncludesGroup(groupIDs []int64, target int64) bool {
...
func betterIncludedRouteBillingSubscription(candidate, current *UserSubscription) bool {
	if candidate == nil {
		return false
	}
	if current == nil {
	...
	candidateLimit := subscriptionLimitScore(candidate.Group)
	currentLimit := subscriptionLimitScore(current.Group)
	if candidateLimit != currentLimit {
		return candidateLimit > currentLimit
	}
...
func subscriptionLimitScore(group *Group) float64 {
	if group == nil {
		return 0
	}
	...
	if group.WeeklyLimitUSD != nil {
		return *group.WeeklyLimitUSD * 4
	}
	if group.DailyLimitUSD != nil {
		return *group.DailyLimitUSD * 30
	}
	return 0
}

func SubscriptionBillingGroup(routeGroup *Group, subscription *UserSubscription) *Group {
	if subscription != nil && subscription.Group != nil && subscription.Group.IsSubscriptionType() {
