...
type AdminService interface {
	...
	// Account management
	ListAccounts(ctx context.Context, page, pageSize int, platform, accountType, status, search string, groupID int64, privacyMode string, source string, sortBy, sortOrder string) ([]Account, int64, error)
		...
		if group.IsSubscriptionType() {
			...
			if _, err := s.userSubRepo.GetActiveByUserIDAndGroupID(ctx, apiKey.UserID, *groupID); err != nil {
				if errors.Is(err, ErrSubscriptionNotFound) {
					includedRouteAllowed, includeErr := subscriptionRouteIncludedByActiveUserPlan(ctx, s.userSubRepo, s.entClient, apiKey.UserID, group.ID)
					if includeErr != nil {
						return nil, includeErr
					}
					balanceFallbackAllowed := false
					if group.AllowsSubscriptionBalanceFallback() && s.userRepo != nil {
						user, userErr := s.userRepo.GetByID(ctx, apiKey.UserID)
						if userErr == nil && CanUseBalanceForSubscriptionGroup(user, group) {
							balanceFallbackAllowed = true
						}
					}
					if !includedRouteAllowed && !balanceFallbackAllowed {
						return nil, infraerrors.BadRequest("SUBSCRIPTION_REQUIRED", "user does not have an active subscription for this group")
					}
					// 允许 Claude 这类订阅线路在无订阅但有余额时绑定，实际请求按余额扣费。
				} else {
					return nil, err
				}
			}
...
func (s *adminServiceImpl) ListAccounts(ctx context.Context, page, pageSize int, platform, accountType, status, search string, groupID int64, privacyMode string, source string, sortBy, sortOrder string) ([]Account, int64, error) {
	params := pagination.PaginationParams{Page: page, PageSize: pageSize, SortBy: sortBy, SortOrder: sortOrder}
	accounts, result, err := s.accountRepo.ListWithFilters(ctx, params, platform, accountType, status, search, groupID, privacyMode, source)
	if err != nil {
		return nil, 0, err
	}
