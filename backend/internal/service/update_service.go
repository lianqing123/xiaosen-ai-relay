...
const (
	...
	customUpdateDisabledWarning = "当前运行的是定制构建，内置在线更新已禁用；请通过定制构建流程合并、构建并部署。"
)
...
func (s *UpdateService) PerformUpdate(ctx context.Context) error {
	if s.isCustomBuild() {
		return fmt.Errorf("%s", customUpdateDisabledWarning)
	}
...
func (s *UpdateService) isCustomBuild() bool {
	version := strings.ToLower(s.currentVersion)
	buildType := strings.ToLower(s.buildType)
	return strings.Contains(version, "-custom") || strings.Contains(buildType, "custom")
}

func (s *UpdateService) customUpdateInfo() *UpdateInfo {
	return &UpdateInfo{
		CurrentVersion: s.currentVersion,
		LatestVersion:  s.currentVersion,
		HasUpdate:      false,
		Cached:         false,
		Warning:        customUpdateDisabledWarning,
		BuildType:      s.buildType,
	}
}
