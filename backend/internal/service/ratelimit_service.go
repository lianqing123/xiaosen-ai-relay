...
func isOpenAI403HTMLChallenge(statusCode int, responseBody []byte) bool {
...
func (s *RateLimitService) handleOpenAI403(ctx context.Context, account *Account, upstreamMsg string, responseBody []byte) (shouldDisable bool) {
	...
	if isOpenAI403HTMLChallenge(http.StatusForbidden, responseBody) {
		until := time.Now().Add(time.Duration(openAI403CooldownMinutesDefault) * time.Minute)
		reason := "OpenAI 403 challenge temporary cooldown: " + msg
		if err := s.accountRepo.SetTempUnschedulable(ctx, account.ID, until, reason); err != nil {
			slog.Warn("openai_403_challenge_set_temp_unschedulable_failed", "account_id", account.ID, "error", err)
			s.handleAuthError(ctx, account, msg)
			return true
		}
		slog.Warn(
			"openai_403_challenge_temp_unschedulable",
