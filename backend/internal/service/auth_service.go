...
func (s *AuthService) IsAffiliateRegistrationInvitationCode(ctx context.Context, code string) bool {
	if s == nil || s.affiliateService == nil {
		return false
	}
	return s.affiliateService.ValidateInviterCodeForRegistration(ctx, code) == nil
}
...
func (s *AuthService) RegisterWithVerification(ctx context.Context, email, password, verifyCode, promoCode, invitationCode, affiliateCode string) (string, *User, error) {
	...
	// 检查是否需要邀请码
	var invitationAffiliateCode string
	if s.settingService != nil && s.settingService.IsInvitationCodeEnabled(ctx) {
		...
		var err error = ErrRedeemCodeNotFound
		if s.redeemRepo != nil {
			redeemCode, err = s.redeemRepo.GetByCode(ctx, invitationCode)
		}
		if err != nil {
			if errors.Is(err, ErrRedeemCodeNotFound) && s.IsAffiliateRegistrationInvitationCode(ctx, invitationCode) {
				invitationAffiliateCode = invitationCode
			} else {
		...
		// 检查类型和状态
		if redeemCode != nil && !IsReusableRegistrationInvitationCode(redeemCode) {
	...
	if s.affiliateService != nil {
		...
		bindAffiliateCode := affiliateCode
		if strings.TrimSpace(bindAffiliateCode) == "" && invitationAffiliateCode != "" {
			bindAffiliateCode = invitationAffiliateCode
		}
		if code := strings.TrimSpace(bindAffiliateCode); code != "" {
	...
	if err != nil {
		if errors.Is(err, ErrUserNotFound) {
			...
			if s.settingService != nil && s.settingService.IsInvitationCodeEnabled(ctx) {
				...
				if !IsReusableRegistrationInvitationCode(redeemCode) {
					return nil, nil, ErrInvitationCodeInvalid
				}
			}
