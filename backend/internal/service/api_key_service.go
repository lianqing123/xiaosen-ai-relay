...
var (
	...
	ErrPreferredSubscriptionNotAllowed = infraerrors.BadRequest("PREFERRED_SUBSCRIPTION_NOT_ALLOWED", "selected subscription is not active for this user")
...
type CreateAPIKeyRequest struct {
	...
	PreferredSubscriptionGroupID *int64   `json:"preferred_subscription_group_id"`
...
// 对于订阅类型分组：优先检查有效订阅；允许余额兜底的分组可用余额绑定。
...
func (s *APIKeyService) canUserBindGroup(ctx context.Context, user *User, group *Group) bool {
	if group.IsSubscriptionType() {
		if s.userSubRepo != nil {
			...
			if ok, err := subscriptionRouteIncludedByActiveUserPlan(ctx, s.userSubRepo, s.entClient, user.ID, group.ID); err == nil && ok {
				return true
			}
		}
		return CanUseBalanceForSubscriptionGroup(user, group)
	}
	// 标准类型分组：使用原有逻辑
	return user.CanBindGroup(group.ID, group.IsExclusive)
}

func (s *APIKeyService) normalizePreferredSubscriptionGroupID(ctx context.Context, user *User, routeGroup *Group, preferredGroupID *int64) (*int64, error) {
	if preferredGroupID == nil || *preferredGroupID <= 0 {
		return nil, nil
	}
	if user == nil || routeGroup == nil || !routeGroup.AllowsSubscriptionBalanceFallback() {
		return nil, nil
	}
	if s.userSubRepo == nil {
		return nil, ErrPreferredSubscriptionNotAllowed
	}
	sub, err := s.userSubRepo.GetActiveByUserIDAndGroupID(ctx, user.ID, *preferredGroupID)
	if err != nil {
		return nil, ErrPreferredSubscriptionNotAllowed
	}
	if sub == nil || sub.Group == nil || !sub.Group.IsActive() || !sub.Group.IsSubscriptionType() {
		return nil, ErrPreferredSubscriptionNotAllowed
	}
	allowed, err := s.preferredSubscriptionCanBillRoute(ctx, routeGroup, sub.GroupID)
	if err != nil {
		return nil, fmt.Errorf("validate preferred subscription: %w", err)
	}
	if !allowed {
		return nil, ErrPreferredSubscriptionNotAllowed
	}
	preferred := *preferredGroupID
	return &preferred, nil
}

func (s *APIKeyService) preferredSubscriptionCanBillRoute(ctx context.Context, routeGroup *Group, preferredGroupID int64) (bool, error) {
	if routeGroup == nil {
		return false, nil
	}
	routeGroupID := routeGroup.ID
	if routeGroupID <= 0 || preferredGroupID <= 0 {
		return false, nil
	}
	if preferredGroupID == routeGroupID {
		return !routeGroup.IsClaudeRoute(), nil
	}
	if s.entClient == nil {
		return false, nil
	}
	plans, err := s.entClient.SubscriptionPlan.Query().
		Where(subscriptionplan.GroupIDEQ(preferredGroupID)).
		All(ctx)
	if err != nil {
		return false, err
	}
...
func (s *APIKeyService) Create(ctx context.Context, userID int64, req CreateAPIKeyRequest) (*APIKey, error) {
	...
	preferredSubscriptionGroupID, err := s.normalizePreferredSubscriptionGroupID(ctx, user, selectedGroup, req.PreferredSubscriptionGroupID)
	if err != nil {
		return nil, err
	}
	...
	apiKey := &APIKey{
		...
		Name:                         req.Name,
		GroupID:                      req.GroupID,
		PreferredSubscriptionGroupID: preferredSubscriptionGroupID,
		Status:                       StatusActive,
...
func (s *APIKeyService) Update(ctx context.Context, id int64, userID int64, req UpdateAPIKeyRequest) (*APIKey, error) {
	...
	if req.PreferredSubscriptionGroupID != nil || req.GroupID != nil {
		user := apiKey.User
		if user == nil {
			var err error
			user, err = s.userRepo.GetByID(ctx, userID)
		...
		apiKey.PreferredSubscriptionGroupID, err = s.normalizePreferredSubscriptionGroupID(ctx, user, apiKey.Group, req.PreferredSubscriptionGroupID)
		if err != nil {
			return nil, err
		}
	}
...
func (s *APIKeyService) GetAvailableGroups(ctx context.Context, userID int64) ([]Group, error) {
	...
	includedRouteGroupIDs, err := includedRouteGroupIDsForSubscribedGroups(ctx, s.entClient, subscribedGroupIDs)
	if err != nil {
		return nil, fmt.Errorf("load included subscription routes: %w", err)
	}

	// 过滤出用户有权限的分组
	availableGroups := make([]Group, 0)
	for _, group := range allGroups {
		if s.canUserBindGroupInternal(user, &group, subscribedGroupIDs, includedRouteGroupIDs) {
...
func (s *APIKeyService) canUserBindGroupInternal(user *User, group *Group, subscribedGroupIDs map[int64]bool, includedRouteGroupIDs map[int64]bool) bool {
	if group.IsSubscriptionType() {
		return subscribedGroupIDs[group.ID] || includedRouteGroupIDs[group.ID] || CanUseBalanceForSubscriptionGroup(user, group)
	}
	// 标准类型分组：使用原有逻辑
	return user.CanBindGroup(group.ID, group.IsExclusive)
}
