...
func TestRateLimitService_HandleUpstreamError_OpenAI403HTMLChallengeStopsFailover(t *testing.T) {
	repo := &rateLimitAccountRepoStub{}
	...
	require.Equal(t, 0, repo.setErrorCalls)
	require.Equal(t, 1, repo.tempCalls)
	require.Contains(t, repo.lastTempReason, "OpenAI 403 challenge temporary cooldown")
	require.Equal(t, []int64{3}, counter.counts, "HTML challenge should not count toward account-disable threshold")
}
