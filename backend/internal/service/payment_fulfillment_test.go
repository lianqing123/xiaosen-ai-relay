...
func TestAffiliateRebateBaseAmountForOrder_IncludesBalanceAndSubscription(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name: "balance order uses CNY payment amount before fee",
			order: &dbent.PaymentOrder{
		...
		{
			name:      "legacy balance order falls back to order amount when payment amount is missing",
			order:     &dbent.PaymentOrder{OrderType: payment.OrderTypeBalance, Amount: 35},
			wantBase:  35,
			wantApply: true,
		},
		{
			name:      "subscription order",
			order:     &dbent.PaymentOrder{OrderType: payment.OrderTypeSubscription, Amount: 99},
			wantBase:  99,
			wantApply: true,
		},
		{
			name:      "zero amount subscription order",
			order:     &dbent.PaymentOrder{OrderType: payment.OrderTypeSubscription, Amount: 0},
			wantBase:  0,
			wantApply: false,
		},
		{
			name:      "unknown order type",
			order:     &dbent.PaymentOrder{OrderType: "manual_adjustment", Amount: 99},
			wantBase:  0,
			wantApply: false,
		},
	}
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			gotBase, gotApply := affiliateRebateBaseAmountForOrder(tt.order)
			assert.Equal(t, tt.wantApply, gotApply)
