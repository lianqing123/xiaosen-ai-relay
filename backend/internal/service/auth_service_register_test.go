...
func newAuthService(repo *userRepoStub, settings map[string]string, emailCache EmailCache) *AuthService {
	return newAuthServiceWithRedeem(repo, settings, emailCache, nil)
}

func newAuthServiceWithRedeem(repo *userRepoStub, settings map[string]string, emailCache EmailCache, redeemRepo RedeemCodeRepository) *AuthService {
	cfg := &config.Config{
...
func (s *affiliateRepoStub) EnsureUserAffiliate(_ context.Context, userID int64) (*AffiliateSummary, error) {
	s.ensuredUserIDs = append(s.ensuredUserIDs, userID)
...
func (s *affiliateRepoStub) GetAffiliateByCode(_ context.Context, code string) (*AffiliateSummary, error) {
	s.getByCodes = append(s.getByCodes, code)
	if s.summariesByCode != nil {
		if summary, ok := s.summariesByCode[code]; ok {
			copied := *summary
			return &copied, nil
		}
	}
...
func (s *affiliateRepoStub) TransferQuotaToBalance(context.Context, int64, float64) (float64, float64, error) {
...
func (s *affiliateRepoStub) ListInviteeRebates(context.Context, AffiliateInviteeRebateFilter) ([]AffiliateInviteeRebateEntry, int64, error) {
	panic("unexpected ListInviteeRebates call")
}

type registrationRedeemCodeRepoStub struct {
...
func (r *registrationRedeemCodeRepoStub) Create(context.Context, *RedeemCode) error {
	panic("unexpected Create call")
}
func (r *registrationRedeemCodeRepoStub) CreateBatch(context.Context, []RedeemCode) error {
	panic("unexpected CreateBatch call")
}
func (r *registrationRedeemCodeRepoStub) GetByID(context.Context, int64) (*RedeemCode, error) {
	panic("unexpected GetByID call")
}
func (r *registrationRedeemCodeRepoStub) GetByCode(_ context.Context, code string) (*RedeemCode, error) {
...
func (r *registrationRedeemCodeRepoStub) Update(context.Context, *RedeemCode) error {
	panic("unexpected Update call")
}
func (r *registrationRedeemCodeRepoStub) Delete(context.Context, int64) error {
	panic("unexpected Delete call")
}
func (r *registrationRedeemCodeRepoStub) Use(_ context.Context, id int64, userID int64) error {
...
func (r *registrationRedeemCodeRepoStub) List(context.Context, pagination.PaginationParams) ([]RedeemCode, *pagination.PaginationResult, error) {
	panic("unexpected List call")
}
func (r *registrationRedeemCodeRepoStub) ListWithFilters(context.Context, pagination.PaginationParams, string, string, string) ([]RedeemCode, *pagination.PaginationResult, error) {
	panic("unexpected ListWithFilters call")
}
func (r *registrationRedeemCodeRepoStub) ListByUser(context.Context, int64, int) ([]RedeemCode, error) {
	panic("unexpected ListByUser call")
}
func (r *registrationRedeemCodeRepoStub) ListByUserPaginated(context.Context, int64, pagination.PaginationParams, string) ([]RedeemCode, *pagination.PaginationResult, error) {
	panic("unexpected ListByUserPaginated call")
}
func (r *registrationRedeemCodeRepoStub) SumPositiveBalanceByUser(context.Context, int64) (float64, error) {
...
func TestAuthService_Register_TrimsInvitationCode(t *testing.T) {
	repo := &userRepoStub{nextID: 12}
	redeemRepo := &registrationRedeemCodeRepoStub{
		codesByCode: map[string]*RedeemCode{
			"INVITE123": {
				ID:     7,
				Code:   "INVITE123",
	...
	service := newAuthServiceWithRedeem(repo, map[string]string{
		SettingKeyRegistrationEnabled:   "true",
		SettingKeyInvitationCodeEnabled: "true",
	}, nil, redeemRepo)

	_, user, err := service.RegisterWithVerification(context.Background(), "user@test.com", "password", "", "", " INVITE123 ", "")

	require.NoError(t, err)
	require.NotNil(t, user)
	require.Equal(t, []string{"INVITE123"}, redeemRepo.getByCodes)
	require.Empty(t, redeemRepo.useCalls)
}

func TestAuthService_Register_UsedInvitationCodeCanBeReused(t *testing.T) {
	usedBy := int64(99)
	usedAt := time.Date(2026, 5, 8, 12, 0, 0, 0, time.UTC)
	...
	_, user, err := service.RegisterWithVerification(context.Background(), "again@test.com", "password", "", "", "INVITE123", "")

	require.NoError(t, err)
	require.NotNil(t, user)
	require.Equal(t, int64(13), user.ID)
	require.Equal(t, []string{"INVITE123"}, redeemRepo.getByCodes)
	require.Empty(t, redeemRepo.useCalls)
}

func TestAuthService_Register_AffiliateCodeCanSatisfyInvitationAndBindInviter(t *testing.T) {
	...
	service := newAuthServiceWithRedeem(repo, settings, nil, redeemRepo)
	affiliateRepo := &affiliateRepoStub{
		summariesByCode: map[string]*AffiliateSummary{
			"AFF123": {UserID: 7, AffCode: "AFF123"},
		},
	}
	service.affiliateService = NewAffiliateService(
		affiliateRepo,
		NewSettingService(&settingRepoStub{values: settings}, service.cfg),
		nil,
		nil,
	)

	_, user, err := service.RegisterWithVerification(context.Background(), "user@test.com", "password", "", "", " aff123 ", "")

	require.NoError(t, err)
	require.NotNil(t, user)
	require.Equal(t, int64(42), user.ID)
	require.Equal(t, []string{"aff123"}, redeemRepo.getByCodes)
	require.Empty(t, redeemRepo.useCalls)
	require.Equal(t, []string{"AFF123", "AFF123"}, affiliateRepo.getByCodes)
