...
type UpdateProfileRequest struct {
	...
	SubscriptionOveragePaygEnabled *bool    `json:"subscription_overage_payg_enabled"`
}
...
func (s *UserService) UpdateProfile(ctx context.Context, userID int64, req UpdateProfileRequest) (*User, error) {
	if txRunner, ok := s.userRepo.(userProfileIdentityTxRunner); ok {
		var (
			updated                           *User
			oldConcurrency                    int
			oldSubscriptionOveragePaygEnabled bool
		)
		if err := txRunner.WithUserProfileIdentityTx(ctx, func(txCtx context.Context) error {
			var err error
			updated, oldConcurrency, oldSubscriptionOveragePaygEnabled, err = s.updateProfile(txCtx, userID, req)
			return err
		}); err != nil {
			return nil, err
		}
		if s.authCacheInvalidator != nil && updated != nil && (updated.Concurrency != oldConcurrency || updated.SubscriptionOveragePaygEnabled != oldSubscriptionOveragePaygEnabled) {
			s.authCacheInvalidator.InvalidateAuthCacheByUserID(ctx, userID)
		}
		return updated, nil
	}

	updated, oldConcurrency, oldSubscriptionOveragePaygEnabled, err := s.updateProfile(ctx, userID, req)
	if err != nil {
		return nil, err
	}
	if s.authCacheInvalidator != nil && (updated.Concurrency != oldConcurrency || updated.SubscriptionOveragePaygEnabled != oldSubscriptionOveragePaygEnabled) {
		s.authCacheInvalidator.InvalidateAuthCacheByUserID(ctx, userID)
	}
	return updated, nil
}
...
func (s *UserService) updateProfile(ctx context.Context, userID int64, req UpdateProfileRequest) (*User, int, bool, error) {
	...
	oldSubscriptionOveragePaygEnabled := user.SubscriptionOveragePaygEnabled
	...
	if req.Email != nil {
		...
		if err != nil {
			return nil, oldConcurrency, oldSubscriptionOveragePaygEnabled, fmt.Errorf("check email exists: %w", err)
		}
		if exists && *req.Email != user.Email {
			return nil, oldConcurrency, oldSubscriptionOveragePaygEnabled, ErrEmailExists
		}
	...
	if req.AvatarURL != nil {
		...
		if err != nil {
			return nil, oldConcurrency, oldSubscriptionOveragePaygEnabled, err
		}
	...
	if req.SubscriptionOveragePaygEnabled != nil {
		user.SubscriptionOveragePaygEnabled = *req.SubscriptionOveragePaygEnabled
	}

	if err := s.userRepo.Update(ctx, user); err != nil {
		return nil, oldConcurrency, oldSubscriptionOveragePaygEnabled, fmt.Errorf("update user: %w", err)
	}

	return user, oldConcurrency, oldSubscriptionOveragePaygEnabled, nil
}
