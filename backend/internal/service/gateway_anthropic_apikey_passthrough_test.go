...
func TestGatewayService_AnthropicAPIKeyPassthrough_ForwardDirect_AddsConservativeCacheControl(t *testing.T) {
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/messages", nil)

	largeContext := strings.Repeat("stable context ", 360)
	body := []byte(`{"model":"claude-sonnet-4-6","messages":[
		{"role":"user","content":[{"type":"text","text":` + mustJSONString(largeContext) + `}]},
	...
	upstream := &anthropicHTTPUpstreamRecorder{
		resp: &http.Response{
			...
			Header: http.Header{
				"Content-Type": []string{"application/json"},
				"x-request-id": []string{"rid-cache-control"},
			},
	...
	result, err := svc.forwardAnthropicAPIKeyPassthrough(context.Background(), c, newAnthropicAPIKeyAccountForTest(), body, "claude-sonnet-4-6", "claude-sonnet-4-6", false, time.Now())
	require.NoError(t, err)
	require.NotNil(t, result)
	require.Equal(t, "ephemeral", gjson.GetBytes(upstream.lastBody, "messages.0.content.0.cache_control.type").String())
	require.Equal(t, "ephemeral", gjson.GetBytes(upstream.lastBody, "messages.3.content.0.cache_control.type").String())
	require.False(t, gjson.GetBytes(upstream.lastBody, "messages.1.content.0.cache_control").Exists())
	require.False(t, gjson.GetBytes(upstream.lastBody, "messages.2.content.0.cache_control").Exists())
}
