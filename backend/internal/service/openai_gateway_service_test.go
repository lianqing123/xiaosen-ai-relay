...
// 编译期接口断言
func TestBuildOpenAICallIDErrorInputDiagnosticsSummarizesTargetWindow(t *testing.T) {
	body := map[string]any{
		"model":  "gpt-5.4",
		"stream": true,
		"input": []any{
			map[string]any{"type": "message", "id": "msg_0", "content": "secret text should not be logged"},
			map[string]any{"type": "reasoning", "id": "rs_1", "summary": []any{"secret reasoning"}},
			map[string]any{"type": "image_generation_call", "id": "ig_2", "call_id": "call_image_2", "status": "completed"},
			map[string]any{"type": "local_shell_call", "id": "sh_3", "call_id": "call_shell_3", "action": "secret command"},
		},
	}
	raw, err := json.Marshal(body)
	require.NoError(t, err)

	diag, ok := buildOpenAICallIDErrorInputDiagnostics("Unknown parameter: 'input[2].call_id'.", raw)
	require.True(t, ok)
	require.NotNil(t, diag)
	require.Equal(t, 2, diag.TargetIndex)
	require.Equal(t, 4, diag.InputLength)
	require.Equal(t, 0, diag.WindowStart)
	require.Equal(t, 3, diag.WindowEnd)
	require.Len(t, diag.Items, 4)
	require.Equal(t, "image_generation_call", diag.Items[2].Type)
	require.True(t, diag.Items[2].HasCallID)
	require.Equal(t, "call", diag.Items[2].CallIDKind)
	require.NotEmpty(t, diag.Items[2].CallIDSHA256)

	encoded, err := json.Marshal(diag)
	require.NoError(t, err)
	require.NotContains(t, string(encoded), "secret")
	require.NotContains(t, string(encoded), "call_image_2")
	require.NotContains(t, string(encoded), "ig_2")
}

func TestBuildOpenAICallIDErrorInputDiagnosticsSkipsUnrelatedErrors(t *testing.T) {
	diag, ok := buildOpenAICallIDErrorInputDiagnostics("Upstream request failed", []byte(`{"input":[]}`))
	require.False(t, ok)
	require.Nil(t, diag)
}
