...
func (s *PaymentService) validateSubOrder(ctx context.Context, req CreateOrderRequest) (*dbent.SubscriptionPlan, error) {
	...
	for _, groupID := range planAdditionalGroupIDs(plan.GroupID, plan.IncludedGroupIds) {
		includedGroup, err := s.groupRepo.GetByID(ctx, groupID)
		if err != nil || includedGroup.Status != payment.EntityStatusActive {
			return nil, infraerrors.NotFound("GROUP_NOT_FOUND", fmt.Sprintf("included subscription group %d is no longer available", groupID))
		}
		if !includedGroup.IsSubscriptionType() {
			return nil, infraerrors.BadRequest("GROUP_TYPE_MISMATCH", fmt.Sprintf("included group %d is not a subscription type", groupID))
		}
	}
	return plan, nil
}
...
func (s *PaymentService) createOrderInTx(ctx context.Context, req CreateOrderRequest, user *User, plan *dbent.SubscriptionPlan, cfg *PaymentConfig, orderAmount, limitAmount, feeRate, payAmount float64, sel *payment.InstanceSelection) (*dbent.PaymentOrder, error) {
	...
	if plan != nil {
		b.SetPlanID(plan.ID).
			...
			SetSubscriptionGroupIds(normalizePlanSubscriptionGroupIDs(plan.GroupID, plan.IncludedGroupIds)).
