...
const openAICallIDDiagnosticWindowRadius = 2

type openAICallIDInputDiagnostic struct {
	...
	Items            []openAICallIDInputDiagnosticItem `json:"items,omitempty"`
}

type openAICallIDInputDiagnosticItem struct {
	...
	IDSHA256      string   `json:"id_sha256,omitempty"`
	HasCallID     bool     `json:"has_call_id,omitempty"`
	CallIDKind    string   `json:"call_id_kind,omitempty"`
	CallIDSHA256  string   `json:"call_id_sha256,omitempty"`
	CallIDValKind string   `json:"call_id_value_kind,omitempty"`
}

func buildOpenAICallIDErrorInputDiagnostics(upstreamMsg string, requestBody []byte) (*openAICallIDInputDiagnostic, bool) {
	targetIndex, ok := extractOpenAICallIDInputIndex(upstreamMsg)
	if !ok {
		return nil, false
	}

	diag := &openAICallIDInputDiagnostic{
		TargetIndex:      targetIndex,
		RequestBodyBytes: len(requestBody),
		ParseStatus:      "ok",
	}
	if len(requestBody) == 0 {
		diag.ParseStatus = "empty_body"
		return diag, true
	}

	var root map[string]any
	if err := json.Unmarshal(requestBody, &root); err != nil {
		diag.ParseStatus = "invalid_json"
		return diag, true
	}

	input, ok := root["input"].([]any)
	if !ok {
		diag.ParseStatus = "input_not_array"
		return diag, true
	}

	diag.InputLength = len(input)
	if len(input) == 0 {
		return diag, true
	}

	start := targetIndex - openAICallIDDiagnosticWindowRadius
	if start < 0 {
		start = 0
	}
	end := targetIndex + openAICallIDDiagnosticWindowRadius
	if end >= len(input) {
		end = len(input) - 1
	}
	if targetIndex < 0 || targetIndex >= len(input) {
		diag.ParseStatus = "target_index_out_of_range"
		start = len(input) - 1
		end = len(input) - 1
	}

	diag.WindowStart = start
	diag.WindowEnd = end
	for i := start; i <= end; i++ {
		diag.Items = append(diag.Items, summarizeOpenAICallIDDiagnosticInputItem(i, input[i]))
	}
	return diag, true
}

func extractOpenAICallIDInputIndex(message string) (int, bool) {
	...
	start += len(prefix)
	rest := message[start:]
	end := strings.Index(rest, "].call_id")
	if end < 0 {
		return 0, false
	}
	rawIndex := strings.TrimSpace(rest[:end])
	if rawIndex == "" {
		return 0, false
	}
...
func summarizeOpenAICallIDDiagnosticInputItem(index int, item any) openAICallIDInputDiagnosticItem {
	out := openAICallIDInputDiagnosticItem{
		Index:     index,
		ValueKind: openAIDiagnosticValueKind(item),
	}
	...
	out.Keys = safeOpenAIDiagnosticKeys(itemMap)
	if idValue, exists := itemMap["id"]; exists {
		out.HasID = true
		if id, ok := idValue.(string); ok {
			out.IDKind = classifyOpenAIDiagnosticID(id)
			out.IDSHA256 = hashSensitiveValueForLog(id)
		}
	}
	if callIDValue, exists := itemMap["call_id"]; exists {
		out.HasCallID = true
		out.CallIDValKind = openAIDiagnosticValueKind(callIDValue)
		if callID, ok := callIDValue.(string); ok {
			out.CallIDKind = classifyOpenAIDiagnosticID(callID)
			out.CallIDSHA256 = hashSensitiveValueForLog(callID)
		}
	}
	return out
}

func safeOpenAIDiagnosticKeys(itemMap map[string]any) []string {
	if len(itemMap) == 0 {
		return nil
	}
...
func classifyOpenAIDiagnosticID(raw string) string {
	value := strings.TrimSpace(raw)
	if value == "" {
		return ""
	}
	for _, prefix := range []string{"call_", "msg_", "rs_", "resp_", "fc", "ig_", "ws_", "sh_", "mcp_"} {
...
func openAIDiagnosticValueKind(value any) string {
	switch value.(type) {
	case nil:
		return "null"
	case string:
...
func logOpenAICallIDErrorInputDiagnostics(ctx context.Context, c *gin.Context, account *Account, upstreamMsg string, requestBody []byte) {
	diag, ok := buildOpenAICallIDErrorInputDiagnostics(upstreamMsg, requestBody)
	if !ok {
		return
	}
	if ctx == nil {
	...
	fields := []zap.Field{
		zap.String("component", "service.openai_gateway"),
		zap.String("upstream_error_message", strings.TrimSpace(upstreamMsg)),
		zap.Any("input_diagnostic", diag),
	}
	if account != nil {
		fields = append(fields,
	...
	if c != nil && c.Request != nil {
		fields = append(fields,
			zap.String("request_path", strings.TrimSpace(c.Request.URL.Path)),
	...
	logger.FromContext(ctx).With(fields...).Warn("OpenAI call_id 参数错误诊断")
}
...
func (s *OpenAIGatewayService) shouldFailoverOpenAIUpstreamResponse(statusCode int, upstreamMsg string, upstreamBody []byte) bool {
	if isOpenAI403HTMLChallenge(statusCode, upstreamBody) {
		return false
	}
...
func (s *OpenAIGatewayService) handleFailoverErrorResponsePassthrough(
	...
	logOpenAICallIDErrorInputDiagnostics(ctx, c, account, upstreamMsg, requestBody)
...
func (s *OpenAIGatewayService) RecordUsage(ctx context.Context, input *OpenAIRecordUsageInput) error {
	...
	if isSubscriptionBilling && s.billingCacheService != nil {
		isSubscriptionBilling = s.billingCacheService.ShouldBillSubscription(ctx, user, apiKey.Group, subscription)
	}
