...
func (s *BillingCacheService) CheckBillingEligibility(ctx context.Context, user *User, apiKey *APIKey, group *Group, subscription *UserSubscription) error {
	...
	// 判断计费模式。group 是路由分组；subscription.Group 可指向该线路
	// 实际应消耗的主套餐分组（例如 Kiro 包含在线路中时）。
	billingGroup := SubscriptionBillingGroup(group, subscription)
	isSubscriptionMode := group != nil && group.IsSubscriptionType() && billingGroup != nil && billingGroup.IsSubscriptionType() && subscription != nil

	if isSubscriptionMode {
		if err := s.checkSubscriptionEligibility(ctx, user.ID, billingGroup, subscription); err != nil {
			if !CanUseBalanceForSubscriptionOverage(user, group) || !isSubscriptionQuotaExceeded(err) {
				return err
			}
...
func (s *BillingCacheService) ShouldBillSubscription(ctx context.Context, user *User, group *Group, subscription *UserSubscription) bool {
	billingGroup := SubscriptionBillingGroup(group, subscription)
	if group == nil || !group.IsSubscriptionType() || billingGroup == nil || !billingGroup.IsSubscriptionType() || subscription == nil {
		return false
	}
	if s == nil || user == nil || !CanUseBalanceForSubscriptionOverage(user, group) {
		return true
	}
	err := s.checkSubscriptionEligibility(ctx, user.ID, billingGroup, subscription)
	return !isSubscriptionQuotaExceeded(err)
}
