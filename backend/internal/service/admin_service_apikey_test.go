...
func TestAdminService_AdminUpdateAPIKeyGroupID_SubscriptionGroup_AllowsIncludedPrimaryPackage(t *testing.T) {
	...
	userSubRepo := &userSubRepoStubForGroupUpdate{
		getActiveErr: ErrSubscriptionNotFound,
		activeSubs: []UserSubscription{{
			ID:        99,
			UserID:    42,
	...
	got, err := svc.AdminUpdateAPIKeyGroupID(ctx, 1, &routeGroup.ID)
	require.NoError(t, err)
	require.NotNil(t, got.APIKey.GroupID)
	require.Equal(t, routeGroup.ID, *got.APIKey.GroupID)
	require.Equal(t, []string{"sk-test"}, cache.keys)
}
