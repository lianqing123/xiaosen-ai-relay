...
type AffiliateInvitee struct {
	...
	TotalRechargedAmount float64    `json:"total_recharged_amount"`
}

type AffiliateInviteeRebateFilter struct {
	Search   string
	Page     int
	PageSize int
}

type AffiliateInviteeRebateEntry struct {
	...
	InviteeCreatedAt     time.Time  `json:"invitee_created_at"`
	TotalRechargedAmount float64    `json:"total_recharged_amount"`
...
type AffiliateRepository interface {
	...
	TransferQuotaToBalance(ctx context.Context, userID int64, balanceMultiplier float64) (float64, float64, error)
	...
	ListInviteeRebates(ctx context.Context, filter AffiliateInviteeRebateFilter) ([]AffiliateInviteeRebateEntry, int64, error)
}
...
// ValidateInviterCodeForRegistration checks whether a referral code can be used
// as a registration invitation code. It does not bind anything.
func (s *AffiliateService) ValidateInviterCodeForRegistration(ctx context.Context, rawCode string) error {
	code := strings.ToUpper(strings.TrimSpace(rawCode))
	if code == "" {
		return ErrAffiliateCodeInvalid
	}
...
func (s *AffiliateService) TransferAffiliateQuota(ctx context.Context, userID int64) (float64, float64, error) {
	...
	balanceMultiplier := defaultBalanceRechargeMultiplier
	if s.settingService != nil {
		balanceMultiplier = s.settingService.GetBalanceRechargeMultiplier(ctx)
	}
	transferred, balance, err := s.repo.TransferQuotaToBalance(ctx, userID, balanceMultiplier)
	if err != nil {
		return 0, 0, err
	}
	if transferred > 0 {
...
// AdminListInviteeRebates returns the actual invitee recharge and rebate totals
// for the admin affiliate dashboard.
func (s *AffiliateService) AdminListInviteeRebates(ctx context.Context, filter AffiliateInviteeRebateFilter) ([]AffiliateInviteeRebateEntry, int64, error) {
	if s == nil || s.repo == nil {
		return nil, 0, infraerrors.ServiceUnavailable("SERVICE_UNAVAILABLE", "affiliate service unavailable")
	}
	return s.repo.ListInviteeRebates(ctx, filter)
}
