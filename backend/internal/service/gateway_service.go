...
func (s *GatewayService) forwardAnthropicAPIKeyPassthroughWithInput(
	...
	input.Body = applyAnthropicPassthroughConservativeCacheControl(input.Body)
	...
	retryStart := time.Now()
	for attempt := 1; attempt <= maxRetryAttempts; attempt++ {
		detachUpstream := input.RequestStream || (!input.RequestStream && len(input.Body) >= 128*1024)
		upstreamCtx, releaseUpstreamCtx := detachStreamUpstreamContext(ctx, detachUpstream)
...
func finalizePostUsageBilling(p *postUsageBillingParams, deps *billingDeps, result *UsageBillingApplyResult) {
	...
	if p.IsSubscriptionBill {
		if p.Cost.ActualCost > 0 && p.User != nil && p.APIKey != nil {
			if groupID, ok := subscriptionUsageCacheGroupID(p); ok {
...
func subscriptionUsageCacheGroupID(p *postUsageBillingParams) (int64, bool) {
	if p != nil && p.Subscription != nil && p.Subscription.GroupID > 0 {
		return p.Subscription.GroupID, true
	}
	if p != nil && p.APIKey != nil && p.APIKey.GroupID != nil && *p.APIKey.GroupID > 0 {
