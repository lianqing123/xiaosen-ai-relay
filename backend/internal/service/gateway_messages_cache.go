...
const anthropicPassthroughAutoCacheMinBytes = 4096

// applyAnthropicPassthroughConservativeCacheControl adds prompt-cache
// breakpoints for API-key passthrough requests without taking over a client's
// existing strategy. It targets only large, likely-stable prefixes.
func applyAnthropicPassthroughConservativeCacheControl(body []byte) []byte {
	if len(body) == 0 {
		return body
	}

	body = addLargeSystemCacheBreakpoint(body)

	tools := gjson.GetBytes(body, "tools")
	if tools.IsArray() && len(tools.Raw) >= anthropicPassthroughAutoCacheMinBytes {
		body = applyToolsLastCacheBreakpoint(body)
	}

	messages := gjson.GetBytes(body, "messages")
	if messages.IsArray() &&
		len(messages.Raw) >= anthropicPassthroughAutoCacheMinBytes &&
...
func addLargeSystemCacheBreakpoint(body []byte) []byte {
	...
	case system.Type == gjson.String:
		text := system.String()
		if len(strings.TrimSpace(text)) < anthropicPassthroughAutoCacheMinBytes {
			return body
		}
		raw := fmt.Sprintf(
	...
	case system.IsArray():
		if len(system.Raw) < anthropicPassthroughAutoCacheMinBytes {
			return body
		}
		arr := system.Array()
		for i := len(arr) - 1; i >= 0; i-- {
			...
			cachePath := fmt.Sprintf("system.%d.cache_control", i)
			if cc := block.Get("cache_control"); cc.Exists() {
				if cc.Get("ttl").String() != "" {
					return body
				}
				if next, err := sjson.SetBytes(body, cachePath+".ttl", claude.DefaultCacheControlTTL); err == nil {
					return next
				}
				return body
			}
			raw := fmt.Sprintf(`{"type":"ephemeral","ttl":%q}`, claude.DefaultCacheControlTTL)
			if next, err := sjson.SetRawBytes(body, cachePath, []byte(raw)); err == nil {
				return next
			}
			return body
		}
...
func messagesHaveCacheControl(messages gjson.Result) bool {
	if !messages.IsArray() {
		return false
	}
	found := false
