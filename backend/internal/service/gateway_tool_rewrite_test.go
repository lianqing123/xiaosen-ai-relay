...
func TestApplyAnthropicPassthroughConservativeCacheControl_SkipsSmallBodies(t *testing.T) {
	body := []byte(`{"system":"short","messages":[{"role":"user","content":"hi"}],"tools":[{"name":"a","input_schema":{}}]}`)
	out := applyAnthropicPassthroughConservativeCacheControl(body)
	require.NotContains(t, string(out), `"cache_control"`)
}

func TestApplyAnthropicPassthroughConservativeCacheControl_CachesLargeStableSections(t *testing.T) {
	largeSystem := strings.Repeat("system prompt ", 400)
	largeToolDescription := strings.Repeat("tool schema ", 500)
	body := []byte(`{
		"system":` + mustJSONString(largeSystem) + `,
		"messages":[{"role":"user","content":"short dynamic user text"}],
		"tools":[{"name":"large_tool","description":` + mustJSONString(largeToolDescription) + `,"input_schema":{"type":"object"}}]
	}`)

	out := applyAnthropicPassthroughConservativeCacheControl(body)

	require.True(t, gjson.GetBytes(out, "system").IsArray())
	require.Equal(t, largeSystem, gjson.GetBytes(out, "system.0.text").String())
...
func TestApplyAnthropicPassthroughConservativeCacheControl_CachesLargeHistoryWithoutOverridingClientStrategy(t *testing.T) {
	largeText := strings.Repeat("conversation context ", 260)
	body := []byte(`{"messages":[
		{"role":"user","content":[{"type":"text","text":` + mustJSONString(largeText) + `}]},
		{"role":"assistant","content":[{"type":"text","text":"a1"}]},
		{"role":"user","content":[{"type":"text","text":"q2"}]},
		{"role":"assistant","content":[{"type":"text","text":"a2"}]}
	]}`)
	...
	clientBody := []byte(`{"messages":[
		{"role":"user","content":[{"type":"text","text":` + mustJSONString(largeText) + `,"cache_control":{"type":"ephemeral","ttl":"1h"}}]},
		{"role":"assistant","content":[{"type":"text","text":"a1"}]},
		{"role":"user","content":[{"type":"text","text":"q2"}]},
		{"role":"assistant","content":[{"type":"text","text":"a2"}]}
	]}`)
	clientOut := applyAnthropicPassthroughConservativeCacheControl(clientBody)
	require.Equal(t, "1h", gjson.GetBytes(clientOut, "messages.0.content.0.cache_control.ttl").String())
	require.False(t, gjson.GetBytes(clientOut, "messages.3.content.0.cache_control").Exists())
}
