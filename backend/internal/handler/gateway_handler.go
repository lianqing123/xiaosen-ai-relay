...
const (
	largeClaudeRequestGuardMinBytes = 128 * 1024
	largeClaudeRequestGuardTTL      = 10 * time.Minute
)

var largeClaudeRequestInFlight sync.Map
...
func claimLargeClaudeRequest(apiKeyID int64, parsedReq *service.ParsedRequest, body []byte) (func(), bool) {
	if parsedReq == nil || parsedReq.Stream || len(body) < largeClaudeRequestGuardMinBytes {
		return nil, true
	}
	if !strings.HasPrefix(strings.ToLower(strings.TrimSpace(parsedReq.Model)), "claude-") {
		return nil, true
	}

	sum := sha256.Sum256(body)
	key := fmt.Sprintf("%d:%s:%s", apiKeyID, strings.ToLower(strings.TrimSpace(parsedReq.Model)), hex.EncodeToString(sum[:]))
	now := time.Now()
	expiresAt := now.Add(largeClaudeRequestGuardTTL)

	if raw, ok := largeClaudeRequestInFlight.Load(key); ok {
		if existing, ok := raw.(time.Time); ok && existing.After(now) {
			return nil, false
		}
		largeClaudeRequestInFlight.Delete(key)
	}

	actual, loaded := largeClaudeRequestInFlight.LoadOrStore(key, expiresAt)
	if loaded {
		if existing, ok := actual.(time.Time); ok && existing.After(now) {
			return nil, false
		}
		largeClaudeRequestInFlight.Store(key, expiresAt)
	}
...
func (h *GatewayHandler) Messages(c *gin.Context) {
	...
	if releaseLargeRequest, acquired := claimLargeClaudeRequest(apiKey.ID, parsedReq, body); !acquired {
		reqLog.Info("gateway.large_claude_duplicate_inflight",
			zap.Int("body_bytes", len(body)),
			zap.String("model", reqModel),
		)
		c.Header("Retry-After", "15")
		h.errorResponse(c, http.StatusTooManyRequests, "rate_limit_error", "duplicate large request is still processing, please retry later")
		return
	} else if releaseLargeRequest != nil {
...
func (h *GatewayHandler) usageUnrestricted(c *gin.Context, ctx context.Context, apiKey *service.APIKey, subject middleware2.AuthSubject, usageData gin.H, modelStats any) {
	...
	if apiKey.Group != nil && apiKey.Group.IsSubscriptionType() {
		...
		if ok && subscription != nil {
			...
			if billingGroup != nil && billingGroup.Name != "" {
				planName = billingGroup.Name
			}
			remaining := h.calculateSubscriptionRemaining(billingGroup, subscription)
			resp := gin.H{
				"mode":        "unrestricted",
			...
			sharedWithSubscription := false
			if apiKey.Group != nil && billingGroup != nil {
				sharedWithSubscription = billingGroup.ID != apiKey.Group.ID
			}
			resp["subscription"] = gin.H{
				"group_id":                 subscription.GroupID,
				"group_name":               planName,
				"route_group_id":           apiKey.Group.ID,
				"route_group_name":         apiKey.Group.Name,
				"shared_with_subscription": sharedWithSubscription,
				"daily_usage_usd":          subscription.DailyUsageUSD,
				"weekly_usage_usd":         subscription.WeeklyUsageUSD,
				"monthly_usage_usd":        subscription.MonthlyUsageUSD,
				"daily_limit_usd":          billingGroup.DailyLimitUSD,
				"weekly_limit_usd":         billingGroup.WeeklyLimitUSD,
				"monthly_limit_usd":        billingGroup.MonthlyLimitUSD,
