...
// ListInviteeRebates returns the real invitee recharge and rebate totals.
// GET /api/v1/admin/affiliates/invitees
func (h *AffiliateHandler) ListInviteeRebates(c *gin.Context) {
	page, pageSize := response.ParsePagination(c)
	search := c.Query("search")

	entries, total, err := h.affiliateService.AdminListInviteeRebates(c.Request.Context(), service.AffiliateInviteeRebateFilter{
		Search:   search,
		Page:     page,
		PageSize: pageSize,
	})
