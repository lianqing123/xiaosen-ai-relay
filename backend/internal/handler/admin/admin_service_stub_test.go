...
type stubAdminService struct {
	...
	lastInactiveBalanceCleanup struct {
		previewReq service.InactiveBalanceCleanupRequest
		executeReq service.InactiveBalanceCleanupRequest
		calls      int
	}
	mu sync.Mutex
}
...
func (s *stubAdminService) ListAccounts(ctx context.Context, page, pageSize int, platform, accountType, status, search string, groupID int64, privacyMode string, source string, sortBy, sortOrder string) ([]service.Account, int64, error) {
...
func (s *stubAdminService) PreviewInactiveBalanceCleanup(ctx context.Context, req service.InactiveBalanceCleanupRequest) (*service.InactiveBalanceCleanupPreview, error) {
	s.lastInactiveBalanceCleanup.previewReq = req
	s.lastInactiveBalanceCleanup.calls++
	now := time.Now().UTC()
	return &service.InactiveBalanceCleanupPreview{
		Days:         3,
		CutoffAt:     now,
		Count:        0,
		TotalBalance: 0,
		Candidates:   []service.InactiveBalanceCleanupCandidate{},
	}, nil
}

func (s *stubAdminService) ExecuteInactiveBalanceCleanup(ctx context.Context, req service.InactiveBalanceCleanupRequest, operatorID int64) (*service.InactiveBalanceCleanupResult, error) {
	s.lastInactiveBalanceCleanup.executeReq = req
	s.lastInactiveBalanceCleanup.calls++
	now := time.Now().UTC()
	return &service.InactiveBalanceCleanupResult{
		Days:           3,
		CutoffAt:       now,
		ClearedCount:   0,
		ClearedBalance: 0,
		Records:        []service.InactiveBalanceCleanupCandidate{},
	}, nil
}
