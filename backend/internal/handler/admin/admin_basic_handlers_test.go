...
func setupAdminRouter() (*gin.Engine, *stubAdminService) {
	...
	router.GET("/api/v1/admin/users/inactive-balance-cleanup/preview", userHandler.PreviewInactiveBalanceCleanup)
	router.POST("/api/v1/admin/users/inactive-balance-cleanup/execute", userHandler.ExecuteInactiveBalanceCleanup)
...
func TestUserHandlerInactiveBalanceCleanupEndpoints(t *testing.T) {
	router, _ := setupAdminRouter()

	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodGet, "/api/v1/admin/users/inactive-balance-cleanup/preview?days=3&limit=50", nil)
