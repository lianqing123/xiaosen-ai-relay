...
type inactiveBalanceCleanupService interface {
	PreviewInactiveBalanceCleanup(ctx context.Context, req service.InactiveBalanceCleanupRequest) (*service.InactiveBalanceCleanupPreview, error)
	ExecuteInactiveBalanceCleanup(ctx context.Context, req service.InactiveBalanceCleanupRequest, operatorID int64) (*service.InactiveBalanceCleanupResult, error)
}
...
type ExecuteInactiveBalanceCleanupRequest struct {
	Days    int    `json:"days"`
...
// PreviewInactiveBalanceCleanup previews active regular users whose balances can be cleared.
// GET /api/v1/admin/users/inactive-balance-cleanup/preview?days=3&limit=50
func (h *UserHandler) PreviewInactiveBalanceCleanup(c *gin.Context) {
	cleanupService, ok := h.adminService.(inactiveBalanceCleanupService)
	if !ok {
		response.InternalError(c, "Inactive balance cleanup service unavailable")
		return
	}

	days, err := parseOptionalIntQuery(c, "days")
	if err != nil {
	...
	preview, err := cleanupService.PreviewInactiveBalanceCleanup(c.Request.Context(), service.InactiveBalanceCleanupRequest{
		Days:  days,
		Limit: limit,
	})
	if err != nil {
...
// ExecuteInactiveBalanceCleanup clears active regular users' balances after the confirmed inactivity window.
// POST /api/v1/admin/users/inactive-balance-cleanup/execute
func (h *UserHandler) ExecuteInactiveBalanceCleanup(c *gin.Context) {
	cleanupService, ok := h.adminService.(inactiveBalanceCleanupService)
	if !ok {
		response.InternalError(c, "Inactive balance cleanup service unavailable")
		return
	...
	var req ExecuteInactiveBalanceCleanupRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.BadRequest(c, "Invalid request: "+err.Error())
		return
	}
	if req.Confirm != service.InactiveBalanceCleanupConfirmToken {
		response.BadRequest(c, "confirm must be "+service.InactiveBalanceCleanupConfirmToken)
		return
	}
	...
	idempotencyPayload := struct {
		...
		Body       ExecuteInactiveBalanceCleanupRequest `json:"body"`
	}{
		OperatorID: subject.UserID,
		Body:       req,
	}
	executeAdminIdempotentJSON(c, "admin.users.inactive_balance_cleanup.execute", idempotencyPayload, service.DefaultWriteIdempotencyTTL(), func(ctx context.Context) (any, error) {
		return cleanupService.ExecuteInactiveBalanceCleanup(ctx, service.InactiveBalanceCleanupRequest{
			Days:  req.Days,
			Limit: req.Limit,
		}, subject.UserID)
	})
