...
func (h *AccountHandler) List(c *gin.Context) {
	...
	accounts, total, err := h.adminService.ListAccounts(c.Request.Context(), page, pageSize, platform, accountType, status, search, groupID, privacyMode, source, sortBy, sortOrder)
	if err != nil {
	...
	etag := buildAccountsListETag(result, total, page, pageSize, platform, accountType, status, search, source, lite)
	if etag != "" {
		c.Header("ETag", etag)
