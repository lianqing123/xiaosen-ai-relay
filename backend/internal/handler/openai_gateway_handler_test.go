...
func TestOpenAIChatCompletionsUsageRecordIncludesRequestPayloadHash(t *testing.T) {
	src, err := os.ReadFile("openai_chat_completions.go")
	require.NoError(t, err)

	require.Contains(t, string(src), "requestPayloadHash := service.HashUsageRequestPayload(body)")
	require.Contains(t, string(src), "RequestPayloadHash: requestPayloadHash")
}
