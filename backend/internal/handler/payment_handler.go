...
func (h *PaymentHandler) GetPlans(c *gin.Context) {
	...
	for _, p := range plans {
		result = append(result, planWithPlatform{
			ID: int64(p.ID), GroupID: p.GroupID, IncludedGroupIDs: p.IncludedGroupIds, GroupPlatform: platformMap[p.GroupID],
...
func (h *PaymentHandler) GetCheckoutInfo(c *gin.Context) {
	...
	for _, p := range plans {
		...
		includedGroupIDs := service.NormalizePlanSubscriptionGroupIDsForDisplay(p.GroupID, p.IncludedGroupIds)
		includedGroups := make([]checkoutPlanIncludedGroup, 0, len(includedGroupIDs))
		for _, groupID := range includedGroupIDs {
			info := groupInfo[groupID]
			includedGroups = append(includedGroups, checkoutPlanIncludedGroup{
				ID:       groupID,
				Name:     info.Name,
		...
		planList = append(planList, checkoutPlan{
			...
			IncludedGroupIDs: includedGroupIDs, IncludedGroups: includedGroups,
...
type checkoutPlan struct {
	...
	GroupID          int64                       `json:"group_id"`
	IncludedGroupIDs []int64                     `json:"included_group_ids"`
	IncludedGroups   []checkoutPlanIncludedGroup `json:"included_groups"`
	GroupPlatform    string                      `json:"group_platform"`
...
type checkoutPlanIncludedGroup struct {
	ID       int64  `json:"id"`
	Name     string `json:"name"`
