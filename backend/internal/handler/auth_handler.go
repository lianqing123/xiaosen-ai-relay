...
func (h *AuthHandler) Register(c *gin.Context) {
	...
	if err := h.authService.CheckRegistrationRisk(c.Request.Context(), service.RegistrationRiskCheckInput{
		Email:     req.Email,
		ClientIP:  clientIP,
	...
	// Turnstile 验证仅保留在最终注册提交阶段。
	if err := h.authService.VerifyTurnstileForRegister(c.Request.Context(), req.TurnstileToken, clientIP, req.VerifyCode); err != nil {
		response.ErrorFrom(c, err)
		return
	}
	...
	h.authService.RecordRegistrationRiskEvent(c.Request.Context(), service.RegistrationRiskRecordInput{
		RegistrationRiskCheckInput: service.RegistrationRiskCheckInput{
			Email:     req.Email,
			ClientIP:  clientIP,
			UserAgent: c.Request.UserAgent(),
		},
...
func (h *AuthHandler) ValidateInvitationCode(c *gin.Context) {
	...
	if err != nil {
		if errors.Is(err, service.ErrRedeemCodeNotFound) && h.authService != nil && h.authService.IsAffiliateRegistrationInvitationCode(c.Request.Context(), req.Code) {
			response.Success(c, ValidateInvitationCodeResponse{
				Valid: true,
			})
			return
	...
	// 检查类型和状态。注册邀请码可重复使用，因此历史 used 状态仍视为有效。
	if !service.IsReusableRegistrationInvitationCode(redeemCode) {
		response.Success(c, ValidateInvitationCodeResponse{
			Valid:     false,
