		...
		if isSubscriptionType && subscriptionService != nil {
			...
			if resolveErr != nil {
				if errors.Is(resolveErr, service.ErrSubscriptionNotFound) {
					abortWithGoogleError(c, 403, "No active package subscription found for this included route")
					return
				}
				abortWithGoogleError(c, 503, "Unable to resolve subscription billing quota")
				return
			}
			if resolvedSub != nil {
			...
			needsMaintenance, err := subscriptionService.ValidateAndCheckLimits(subscription, billingGroup)
			if err != nil && !shouldDeferSubscriptionQuotaToBilling(apiKey.User, apiKey.Group, err) {
				status := 403
