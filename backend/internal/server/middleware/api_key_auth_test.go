...
func TestSimpleModeBypassesQuotaCheck(t *testing.T) {
	...
	t.Run("standard_mode_defers_subscription_quota_to_billing_when_overage_payg_enabled", func(t *testing.T) {
		cfg := &config.Config{RunMode: config.RunModeStandard}
		overageUser := *user
		overageUser.SubscriptionOveragePaygEnabled = true
		overageUser.Balance = 5
		overageAPIKey := *apiKey
		overageAPIKey.User = &overageUser

		apiKeyService := service.NewAPIKeyService(&stubApiKeyRepo{
			getByKey: func(ctx context.Context, key string) (*service.APIKey, error) {
				if key != overageAPIKey.Key {
					return nil, service.ErrAPIKeyNotFound
				}
				clone := overageAPIKey
				return &clone, nil
		...
		w := httptest.NewRecorder()
		req := httptest.NewRequest(http.MethodGet, "/t", nil)
		req.Header.Set("x-api-key", overageAPIKey.Key)
		router.ServeHTTP(w, req)

		require.Equal(t, http.StatusOK, w.Code)
	})

	t.Run("standard_mode_claude_subscription_quota_uses_unified_overage_switch", func(t *testing.T) {
		cfg := &config.Config{RunMode: config.RunModeStandard}
		claudeGroup := &service.Group{
			ID:                   24,
			Name:                 "Claude",
			Status:               service.StatusActive,
		...
		tests := []struct {
			...
			{name: "disabled", overagePayg: false, wantHTTPCode: http.StatusTooManyRequests},
			{name: "enabled", overagePayg: true, wantHTTPCode: http.StatusOK},
		}

		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				claudeUser := *user
				claudeUser.Balance = 5
				claudeUser.SubscriptionOveragePaygEnabled = tt.overagePayg
				claudeAPIKey := *apiKey
				claudeAPIKey.User = &claudeUser
				claudeAPIKey.Group = claudeGroup
				claudeAPIKey.GroupID = &claudeGroup.ID

				apiKeyService := service.NewAPIKeyService(&stubApiKeyRepo{
					getByKey: func(ctx context.Context, key string) (*service.APIKey, error) {
						if key != claudeAPIKey.Key {
							return nil, service.ErrAPIKeyNotFound
						}
						clone := claudeAPIKey
						return &clone, nil
				...
				w := httptest.NewRecorder()
				req := httptest.NewRequest(http.MethodGet, "/t", nil)
				req.Header.Set("x-api-key", claudeAPIKey.Key)
				router.ServeHTTP(w, req)
	...
	t.Run("standard_mode_subscription_ignores_negative_balance_inside_quota", func(t *testing.T) {
		cfg := &config.Config{RunMode: config.RunModeStandard}
		negativeBalanceUser := *user
		negativeBalanceUser.Balance = -0.03
		negativeBalanceAPIKey := *apiKey
		negativeBalanceAPIKey.User = &negativeBalanceUser

		apiKeyService := service.NewAPIKeyService(&stubApiKeyRepo{
			getByKey: func(ctx context.Context, key string) (*service.APIKey, error) {
				if key != negativeBalanceAPIKey.Key {
					return nil, service.ErrAPIKeyNotFound
				}
				clone := negativeBalanceAPIKey
				return &clone, nil
			},
		}, nil, nil, nil, nil, nil, nil, cfg)
		...
		w := httptest.NewRecorder()
		req := httptest.NewRequest(http.MethodGet, "/t", nil)
		req.Header.Set("x-api-key", negativeBalanceAPIKey.Key)
		router.ServeHTTP(w, req)
...
func TestAPIKeyAuthZeroBalanceStandardKeyFallsBackToActiveSubscription(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	router.GET("/t", func(c *gin.Context) {
		...
		groupFromCtx, ok := c.Request.Context().Value(ctxkey.Group).(*service.Group)
		require.True(t, ok)
		require.Equal(t, subscriptionGroup.ID, groupFromCtx.ID)

		gotKey, ok := GetAPIKeyFromContext(c)
		require.True(t, ok)
		require.NotNil(t, gotKey.GroupID)
		require.Equal(t, subscriptionGroup.ID, *gotKey.GroupID)
...
func TestAPIKeyAuthSubscriptionClaudeAllowsPositiveBalanceFallback(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	tests := []struct {
		...
		{name: "positive balance can use Claude without subscription", balance: 1, wantStatus: http.StatusOK},
		{name: "zero balance still needs subscription", balance: 0, wantStatus: http.StatusForbidden},
	}
...
func TestAPIKeyAuthClaudeDirectRouteSubscriptionFallsBackToBalanceWhenPrimaryPackageMissing(t *testing.T) {
	gin.SetMode(gin.TestMode)
	ctx := context.Background()
	client := newAPIKeyAuthMiddlewareTestClient(t)

	primaryGroupEnt, err := client.Group.Create().
		SetName("Codex Package").
	...
	routeGroupEnt, err := client.Group.Create().
		SetName("Claude").
	...
	_, err = client.SubscriptionPlan.Create().
		SetName("Codex with Claude").
		SetGroupID(primaryGroupEnt.ID).
		SetIncludedGroupIds([]int64{routeGroupEnt.ID}).
		SetPrice(399).
		SetValidityDays(30).
		Save(ctx)
	require.NoError(t, err)
	...
	now := time.Now()
	routeSub := service.UserSubscription{
		ID:        58,
		UserID:    user.ID,
		GroupID:   routeGroup.ID,
		Status:    service.SubscriptionStatusActive,
	...
	subscriptionService := service.NewSubscriptionService(nil, &stubUserSubscriptionRepo{
		getActive: func(ctx context.Context, userID, groupID int64) (*service.UserSubscription, error) {
			require.Equal(t, user.ID, userID)
			require.Equal(t, routeGroup.ID, groupID)
			clone := routeSub
			return &clone, nil
		},
		listActive: func(ctx context.Context, userID int64) ([]service.UserSubscription, error) {
			require.Equal(t, user.ID, userID)
			return []service.UserSubscription{routeSub}, nil
		},
	}, nil, client, cfg)
...
func newAPIKeyAuthMiddlewareTestClient(t *testing.T) *dbent.Client {
	t.Helper()
