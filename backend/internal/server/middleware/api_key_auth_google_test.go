...
func TestApiKeyAuthWithSubscriptionGoogle_DefersQuotaExceededWhenOveragePaygEnabled(t *testing.T) {
	gin.SetMode(gin.TestMode)

	limit := 1.0
	group := &service.Group{
		ID:               78,
		Name:             "gemini-sub-payg",
		Status:           service.StatusActive,
		Platform:         service.PlatformGemini,
		Hydrated:         true,
		SubscriptionType: service.SubscriptionTypeSubscription,
	...
	user := &service.User{
		...
		Balance:                        5,
		Concurrency:                    3,
		SubscriptionOveragePaygEnabled: true,
	}
	apiKey := &service.APIKey{
		ID:     502,
		UserID: user.ID,
