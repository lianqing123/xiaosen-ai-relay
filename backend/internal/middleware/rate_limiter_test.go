...
func TestRateLimiterUsesForwardedRealIP(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	router.Use(limiter.Limit("auth-register", 1, time.Second))
	router.GET("/test", func(c *gin.Context) {
		c.JSON(http.StatusOK, gin.H{"ok": true})
	})
	...
	req1.Header.Set("CF-Connecting-IP", "8.8.8.8")
	rec1 := httptest.NewRecorder()
	router.ServeHTTP(rec1, req1)
	...
	req3.Header.Set("CF-Connecting-IP", "8.8.8.8")
	rec3 := httptest.NewRecorder()
	router.ServeHTTP(rec3, req3)
	require.Equal(t, http.StatusTooManyRequests, rec3.Code)

	require.Contains(t, callCounts, "rate_limit:auth-register:8.8.8.8")
	require.Contains(t, callCounts, "rate_limit:auth-register:1.1.1.1")
	require.NotContains(t, callCounts, "rate_limit:auth-register:127.0.0.1")
}
