...
func (s *AccountRepoSuite) TestListWithFilters() {
	tests := []struct {
		...
		{
			name: "filter_by_kiro_source",
			setup: func(client *dbent.Client) {
				mustCreateAccount(s.T(), client, &service.Account{Name: "kiro-source", Extra: map[string]any{"source": service.AccountSourceKiroGateway}})
				mustCreateAccount(s.T(), client, &service.Account{Name: "kiro-deploy", Extra: map[string]any{"deployment_path": "/opt/kiro-gateway"}})
				mustCreateAccount(s.T(), client, &service.Account{Name: "normal", Extra: map[string]any{"source": "other"}})
			},
			source:    service.AccountSourceKiroGateway,
			wantCount: 2,
			validate: func(accounts []service.Account) {
				names := []string{accounts[0].Name, accounts[1].Name}
				s.ElementsMatch([]string{"kiro-source", "kiro-deploy"}, names)
			},
		},
	}
	...
	for _, tt := range tests {
		s.Run(tt.name, func() {
			...
			accounts, _, err := repo.ListWithFilters(ctx, pagination.PaginationParams{Page: 1, PageSize: 10}, tt.platform, tt.accType, tt.status, tt.search, tt.groupID, tt.privacyMode, tt.source)
			s.Require().NoError(err)
