...
func (r *accountRepository) ListWithFilters(ctx context.Context, params pagination.PaginationParams, platform, accountType, status, search string, groupID int64, privacyMode string, source string) ([]service.Account, *pagination.PaginationResult, error) {
	...
	if source != "" {
		q = q.Where(accountSourcePredicate(source))
	}
...
func accountSourcePredicate(source string) dbpredicate.Account {
	source = strings.TrimSpace(source)
	return dbpredicate.Account(func(s *entsql.Selector) {
		sourcePath := sqljson.Path("source")
		if source == service.AccountSourceKiroGateway {
			s.Where(entsql.Or(
				sqljson.ValueEQ(dbaccount.FieldExtra, service.AccountSourceKiroGateway, sourcePath),
				sqljson.ValueEQ(dbaccount.FieldExtra, "/opt/kiro-gateway", sqljson.Path("deployment_path")),
			))
			return
		}
		s.Where(sqljson.ValueEQ(dbaccount.FieldExtra, source, sourcePath))
	})
}
