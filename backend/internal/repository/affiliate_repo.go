...
func (r *affiliateRepository) TransferQuotaToBalance(ctx context.Context, userID int64, balanceMultiplier float64) (float64, float64, error) {
...
FROM cleared`, userID)
		...
		creditedBalance := calculateAffiliateBalanceCredit(transferred, balanceMultiplier)
		...
		affected, err := txClient.User.Update().
			...
			AddTotalRecharged(creditedBalance).
			Save(txCtx)
		if err != nil {
...
func calculateAffiliateBalanceCredit(rebateAmountCNY, multiplier float64) float64 {
	if rebateAmountCNY <= 0 || math.IsNaN(rebateAmountCNY) || math.IsInf(rebateAmountCNY, 0) {
		return 0
	}
	if multiplier <= 0 || math.IsNaN(multiplier) || math.IsInf(multiplier, 0) {
		multiplier = 1
	}
	return decimal.NewFromFloat(rebateAmountCNY).
...
func (r *affiliateRepository) ListInvitees(ctx context.Context, inviterID int64, limit int) ([]service.AffiliateInvitee, error) {
	client := clientFromContext(ctx, r.client)
	return listAffiliateInviteesWithClient(ctx, client, inviterID, limit)
}

func listAffiliateInviteesWithClient(ctx context.Context, client affiliateQueryExecer, inviterID int64, limit int) ([]service.AffiliateInvitee, error) {
	if limit <= 0 {
		limit = 100
	}
	rows, err := client.QueryContext(ctx, `
SELECT ua.user_id,
       ...
       COALESCE(recharge.total_recharged_amount, 0)::double precision AS total_recharged_amount
...
LEFT JOIN (
    SELECT user_id,
           SUM(CASE
               WHEN order_type = 'balance' THEN
                   CASE
                       WHEN pay_amount > 0 THEN pay_amount / (1 + GREATEST(fee_rate, 0) / 100)
                       ELSE amount
                   END
               ELSE amount
           END)::double precision AS total_recharged_amount
    FROM payment_orders
    WHERE order_type IN ('balance', 'subscription') AND status = 'COMPLETED'
    GROUP BY user_id
) recharge ON recharge.user_id = ua.user_id
...
LIMIT $2`, inviterID, limit)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.UserID, &item.Email, &item.Username, &createdAt, &item.TotalRebate, &item.TotalRechargedAmount); err != nil {
			return nil, err
		}
...
func (r *affiliateRepository) ListInviteeRebates(ctx context.Context, filter service.AffiliateInviteeRebateFilter) ([]service.AffiliateInviteeRebateEntry, int64, error) {
	client := clientFromContext(ctx, r.client)
	return listAffiliateInviteeRebatesWithClient(ctx, client, filter)
}

func listAffiliateInviteeRebatesWithClient(ctx context.Context, client affiliateQueryExecer, filter service.AffiliateInviteeRebateFilter) ([]service.AffiliateInviteeRebateEntry, int64, error) {
	page := filter.Page
	if page < 1 {
		page = 1
	}
...
WHERE ua.inviter_id IS NOT NULL
  AND (
    inviter.email ILIKE $1
    OR inviter.username ILIKE $2
    OR invitee.email ILIKE $3
    OR invitee.username ILIKE $4
    OR COALESCE(inviter_aff.aff_code, '') ILIKE $5
  )`

	total, err := scanInt64(ctx, client, "SELECT COUNT(*)"+baseJoin+baseWhere, args...)
	if err != nil {
		return nil, 0, fmt.Errorf("count affiliate invitee rebates: %w", err)
	}
	...
	rows, err := client.QueryContext(ctx, `SELECT ua.inviter_id,
       ...
       ua.created_at,
       COALESCE(recharge.total_recharged_amount, 0)::double precision,
       COALESCE(recharge.total_paid_amount, 0)::double precision,
       COALESCE(rebate.total_rebate_amount, 0)::double precision,
...
LEFT JOIN (
    SELECT user_id,
           SUM(CASE
               ...
               ELSE amount
           END)::double precision AS total_recharged_amount,
           SUM(pay_amount)::double precision AS total_paid_amount,
...
LEFT JOIN (
    SELECT user_id AS inviter_id,
           source_user_id AS invitee_id,
           SUM(amount)::double precision AS total_rebate_amount
    FROM user_affiliate_ledger
    WHERE action = 'accrue'
    GROUP BY user_id, source_user_id
) rebate ON rebate.inviter_id = ua.inviter_id AND rebate.invitee_id = ua.user_id
`+baseWhere+`
ORDER BY recharge.last_recharge_at DESC NULLS LAST, ua.created_at DESC, ua.user_id DESC
LIMIT $6 OFFSET $7`, listArgs...)
	if err != nil {
		return nil, 0, fmt.Errorf("list affiliate invitee rebates: %w", err)
	}
	defer func() { _ = rows.Close() }()

	entries := make([]service.AffiliateInviteeRebateEntry, 0)
	for rows.Next() {
		var entry service.AffiliateInviteeRebateEntry
		var lastRechargeAt sql.NullTime
		if err := rows.Scan(
			&entry.InviterID,
			&entry.InviterEmail,
			&entry.InviterUsername,
			&entry.InviterAffCode,
			&entry.InviteeID,
			&entry.InviteeEmail,
			&entry.InviteeUsername,
			&entry.InviteeCreatedAt,
			&entry.TotalRechargedAmount,
			&entry.TotalPaidAmount,
			&entry.TotalRebateAmount,
			&lastRechargeAt,
		); err != nil {
			return nil, 0, err
		}
		if lastRechargeAt.Valid {
			entry.LastRechargeAt = &lastRechargeAt.Time
		}
