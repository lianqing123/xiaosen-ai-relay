  ...
  >
    <form id="provider-form" @submit.prevent="handleSave" class="space-y-4">
      ...
      <div class="flex flex-wrap items-center gap-x-5 gap-y-2">
        ...
        <div v-if="form.provider_key === 'easypay' || form.provider_key === 'usdc'" class="flex items-center gap-2">
          ...
          <div class="flex gap-1.5">
            <button
              ...
              :key="mode.value"
              type="button"
              :disabled="form.provider_key === 'usdc'"
              @click="form.provider_key !== 'usdc' && (form.payment_mode = mode.value)"
              :class="[
...
const EASY_PAY_FIELD_LABELS: Record<string, string> = {
  epayVersion: '协议版本',
  pkey: '商户密钥(v1)',
  privateKey: '商户私钥(v2)',
  publicKey: '平台公钥(v2)',
...
const resolvedFields = computed(() => {
  ...
  return fields.map(f => ({
    ...f,
    label: resolveProviderFieldLabel(form.provider_key, f.key, f.label),
  }))
})
...
function resolveProviderFieldLabel(providerKey: string, key: string, fallback: string): string {
  if (providerKey === 'easypay' && EASY_PAY_FIELD_LABELS[key]) {
    return EASY_PAY_FIELD_LABELS[key]
  }
  return fallback || t(`admin.settings.payment.field_${key}`)
}

function normalizePaymentMode(mode: string, providerKey: string): string {
  if (providerKey === 'usdc') return PAYMENT_MODE_QRCODE
  if (providerKey !== 'easypay') return ''
  if (mode === PAYMENT_MODE_REDIRECT) return PAYMENT_MODE_POPUP
  if (mode === PAYMENT_MODE_POPUP || mode === PAYMENT_MODE_QRCODE) return mode
  return PAYMENT_MODE_QRCODE
}
...
function handleSave() {
  ...
  for (const f of PROVIDER_CONFIG_FIELDS[form.provider_key] || []) {
    ...
    if (!val) {
      const label = resolveProviderFieldLabel(form.provider_key, f.key, f.label)
  ...
  emit('save', {
    ...
    payment_mode: form.provider_key === 'easypay' || form.provider_key === 'usdc' ? form.payment_mode : '',
...
function reset(defaultKey: string) {
  ...
  form.enabled = true
  form.payment_mode = normalizePaymentMode('', defaultKey)
...
function loadProvider(provider: ProviderInstance) {
  ...
  form.payment_mode = normalizePaymentMode(provider.payment_mode || '', provider.provider_key)
