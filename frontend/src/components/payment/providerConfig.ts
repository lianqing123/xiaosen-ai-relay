...
export const METHOD_ORDER = ['alipay', 'alipay_direct', 'wxpay', 'wxpay_direct', 'usdc', 'stripe'] as const
...
export const WEBHOOK_PATHS: Record<string, string> = {
  ...
  usdc: '/api/v1/payment/webhook/usdc',
}
...
export const PROVIDER_CONFIG_FIELDS: Record<string, ConfigFieldDef[]> = {
  easypay: [
    ...
    { key: 'epayVersion', label: '协议版本', sensitive: false, defaultValue: 'v2' },
    { key: 'pkey', label: '商户密钥(v1)', sensitive: true, optional: true },
    { key: 'privateKey', label: '商户私钥(v2)', sensitive: true, optional: true },
    { key: 'publicKey', label: '平台公钥(v2)', sensitive: false, optional: true },
    { key: 'apiBase', label: '网关地址', sensitive: false },
    { key: 'apiPath', label: '创建接口路径', sensitive: false, optional: true, defaultValue: '/api/pay/create' },
    { key: 'signType', label: '签名类型', sensitive: false, optional: true, defaultValue: 'RSA' },
    { key: 'method', label: '下单方式', sensitive: false, optional: true, defaultValue: 'web' },
    { key: 'cidAlipay', label: '支付宝 CID', sensitive: false, optional: true },
    { key: 'cidWxpay', label: '微信 CID', sensitive: false, optional: true },
  ],
  alipay: [
  ...
  usdc: [
    { key: 'address', label: '', sensitive: false },
    { key: 'network', label: '', sensitive: false, defaultValue: 'Base' },
    { key: 'cnyPerUsdc', label: '', sensitive: false },
    { key: 'memoPrefix', label: '', sensitive: false, optional: true, defaultValue: 'zz' },
    { key: 'qrTemplate', label: '', sensitive: false, optional: true },
    { key: 'webhookSecret', label: '', sensitive: true, optional: true },
  ],
}
