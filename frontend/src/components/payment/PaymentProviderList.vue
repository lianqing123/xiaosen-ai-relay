      ...
      >
        <div v-for="p in localProviders" :key="p.id" class="flex items-start gap-2">
          ...
          <div class="min-w-0 flex-1">
            <ProviderCard
              :provider="p"
              :enabled="isProviderGloballyEnabled(p)"
...
function normalizePaymentType(type: string): string {
  ...
  if (type === 'card' || type === 'link') return 'stripe'
  return type
}

function isProviderGloballyEnabled(provider: ProviderInstance): boolean {
  ...
  return supportedTypes.some((type) => props.enabledPaymentTypes.includes(normalizePaymentType(type)))
}
