...
function isSuccessStatus(status: string | null | undefined, orderType?: string): boolean {
...
async function fetchLatestOrder(): Promise<PaymentOrder | null> {
  const outTradeNo = props.outTradeNo?.trim()
  if (props.orderType !== 'subscription' || !outTradeNo) {
  ...
  try {
    ...
    console.warn('[payment] verify order failed, falling back to local polling', err)
    return paymentStore.pollOrderStatus(props.orderId)
  }
}
...
async function pollStatus() {
  ...
  if (!order) return
  if (isSuccessStatus(order.status, order.order_type || props.orderType)) {
    cleanup()
