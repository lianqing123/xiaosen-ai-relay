...
describe('PaymentStatusPanel', () => {
  ...
  it('does not report subscription success before fulfillment is completed', async () => {
    pollOrderStatus.mockResolvedValue(orderFactory('PAID', 'subscription'))
  ...
  it('verifies the provider order by out_trade_no so subscription fulfillment can run after payment', async () => {
    verifyOrder.mockResolvedValue({ data: orderFactory('COMPLETED', 'subscription') })
