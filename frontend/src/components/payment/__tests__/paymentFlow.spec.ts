...
describe('getVisibleMethods', () => {
  ...
  it('keeps USDC as a user-facing hosted payment method', () => {
    const visible = getVisibleMethods({
      usdc: methodLimit({ fee_rate: 1 }),
    })
...
describe('decidePaymentLaunch', () => {
  ...
  it('uses QR waiting flow for USDC hosted checkout responses', () => {
    const decision = decidePaymentLaunch(createOrderResult({
      payment_type: 'usdc',
      payment_mode: 'qrcode',
      qr_code: 'usdc:0xmerchant?amount=10',
    }), {
      visibleMethod: 'usdc',
      orderType: 'balance',
      isMobile: false,
    ...
    expect(decision.kind).toBe('qr_waiting')
    expect(decision.paymentState.paymentType).toBe('usdc')
    expect(decision.paymentState.qrCode).toBe('usdc:0xmerchant?amount=10')
  })
})
...
describe('buildCreateOrderPayload', () => {
  ...
  it('sends USDC as the selected hosted payment type', () => {
    expect(buildCreateOrderPayload({
      amount: 20,
      paymentType: 'usdc',
      orderType: 'balance',
