...
const KIRO_GROUP_ID = 24
const KIRO_CLAUDE_MODELS = {
...
const isKiroGroup = computed(() => {
  if (props.platform !== 'anthropic') return false
  const groupName = props.groupName || ''
  return props.groupId === KIRO_GROUP_ID || /kiro/i.test(groupName) || groupName.includes('Kiro')
})
...
function generateAnthropicFiles(baseUrl: string, apiKey: string): FileConfig[] {
  let path: string
  let content: string
  const kiroEnvLines = isKiroGroup.value
    ? {
        unix: `export ANTHROPIC_API_KEY="${apiKey}"
export ANTHROPIC_MODEL="${KIRO_CLAUDE_MODELS.main}"
export ANTHROPIC_DEFAULT_SONNET_MODEL="${KIRO_CLAUDE_MODELS.sonnet}"
export ANTHROPIC_DEFAULT_HAIKU_MODEL="${KIRO_CLAUDE_MODELS.haiku}"
export ANTHROPIC_DEFAULT_OPUS_MODEL="${KIRO_CLAUDE_MODELS.opus}"`,
        cmd: `set ANTHROPIC_API_KEY=${apiKey}
set ANTHROPIC_MODEL=${KIRO_CLAUDE_MODELS.main}
set ANTHROPIC_DEFAULT_SONNET_MODEL=${KIRO_CLAUDE_MODELS.sonnet}
set ANTHROPIC_DEFAULT_HAIKU_MODEL=${KIRO_CLAUDE_MODELS.haiku}
set ANTHROPIC_DEFAULT_OPUS_MODEL=${KIRO_CLAUDE_MODELS.opus}`,
        powershell: `$env:ANTHROPIC_API_KEY="${apiKey}"
$env:ANTHROPIC_MODEL="${KIRO_CLAUDE_MODELS.main}"
$env:ANTHROPIC_DEFAULT_SONNET_MODEL="${KIRO_CLAUDE_MODELS.sonnet}"
$env:ANTHROPIC_DEFAULT_HAIKU_MODEL="${KIRO_CLAUDE_MODELS.haiku}"
$env:ANTHROPIC_DEFAULT_OPUS_MODEL="${KIRO_CLAUDE_MODELS.opus}"`
      }
    : null
...
${kiroEnvLines ? `${kiroEnvLines.unix}\n` : ''}export CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC=1`
      break
    case 'cmd':
      path = 'Command Prompt'
      content = `set ANTHROPIC_BASE_URL=${baseUrl}
set ANTHROPIC_AUTH_TOKEN=${apiKey}
${kiroEnvLines ? `${kiroEnvLines.cmd}\n` : ''}set CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC=1`
      break
    case 'powershell':
      path = 'PowerShell'
      content = `$env:ANTHROPIC_BASE_URL="${baseUrl}"
$env:ANTHROPIC_AUTH_TOKEN="${apiKey}"
${kiroEnvLines ? `${kiroEnvLines.powershell}\n` : ''}$env:CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC=1`
      break
    default:
      path = 'Terminal'
      content = ''
  ...
  if (isKiroGroup.value) {
    Object.assign(vscodeEnv, {
      ANTHROPIC_API_KEY: apiKey,
      ANTHROPIC_MODEL: KIRO_CLAUDE_MODELS.main,
      ANTHROPIC_DEFAULT_SONNET_MODEL: KIRO_CLAUDE_MODELS.sonnet,
      ANTHROPIC_DEFAULT_HAIKU_MODEL: KIRO_CLAUDE_MODELS.haiku,
      ANTHROPIC_DEFAULT_OPUS_MODEL: KIRO_CLAUDE_MODELS.opus
    })
  }

  const vscodeContent = JSON.stringify({ env: vscodeEnv }, null, 2)
...
function generateOpenCodeConfig(platform: string, baseUrl: string, apiKey: string, pathLabel?: string): FileConfig {
  ...
  if (platform === 'gemini') {
    ...
    if (isKiroGroup.value) {
      provider[platform].name = 'Kiro Gateway'
      provider[platform].models = kiroClaudeModels
    }
