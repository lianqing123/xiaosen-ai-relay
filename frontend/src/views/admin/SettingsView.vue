    ...
    <div class="mx-auto max-w-4xl space-y-6">
      ...
      <form v-else @submit.prevent="saveSettings" class="space-y-6" novalidate>
        ...
        <div v-show="activeTab === 'security'" class="space-y-6">
          ...
          <div class="card">
            ...
            <div class="space-y-5 p-6">
              ...
              <!-- Password reset uses email but should be configurable independently. -->
              <div
                class="flex items-center justify-between border-t border-gray-100 pt-4 dark:border-dark-700"
              >
                <div>
          ...
          <div class="space-y-5 p-6">
            ...
            <div v-if="form.affiliate_enabled" class="space-y-6">
              ...
              <div class="border-t border-gray-100 pt-6 dark:border-dark-700">
                <div class="mb-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <h3 class="text-sm font-semibold text-gray-900 dark:text-white">
                      {{ t('admin.settings.features.affiliate.inviteeRebates.title') }}
                    </h3>
                    <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                      {{ t('admin.settings.features.affiliate.inviteeRebates.description') }}
                    </p>
                  </div>
                  <button
                    type="button"
                    class="btn btn-secondary btn-sm"
                    :disabled="affiliateInviteeRebateState.loading"
                    @click="loadAffiliateInviteeRebates"
                  >
                    {{ t('common.refresh') }}
                  </button>
                </div>
                ...
                <div class="mb-3">
                  <input
                    v-model="affiliateInviteeRebateState.search"
                    type="text"
                    class="input"
                    :placeholder="t('admin.settings.features.affiliate.inviteeRebates.searchPlaceholder')"
                    @input="onAffiliateInviteeRebateSearchInput"
                  />
                </div>

                <div class="overflow-hidden rounded-2xl border border-stone-200/80 bg-white/70 shadow-[inset_0_1px_0_rgba(255,255,255,0.7)] dark:border-dark-700 dark:bg-dark-900/30">
                  <table class="min-w-full divide-y divide-stone-200/80 dark:divide-dark-700">
                    <thead class="bg-stone-50/90 dark:bg-dark-800">
                      <tr>
                        <th class="px-3 py-2.5 text-left text-xs font-medium uppercase tracking-wide text-stone-500">{{ t('admin.settings.features.affiliate.inviteeRebates.col.inviter') }}</th>
                        <th class="px-3 py-2.5 text-left text-xs font-medium uppercase tracking-wide text-stone-500">{{ t('admin.settings.features.affiliate.inviteeRebates.col.invitee') }}</th>
                        <th class="px-3 py-2.5 text-right text-xs font-medium uppercase tracking-wide text-stone-500">{{ t('admin.settings.features.affiliate.inviteeRebates.col.recharged') }}</th>
                        <th class="px-3 py-2.5 text-right text-xs font-medium uppercase tracking-wide text-stone-500">{{ t('admin.settings.features.affiliate.inviteeRebates.col.paid') }}</th>
                        <th class="px-3 py-2.5 text-right text-xs font-medium uppercase tracking-wide text-stone-500">{{ t('admin.settings.features.affiliate.inviteeRebates.col.rebate') }}</th>
                        <th class="px-3 py-2.5 text-left text-xs font-medium uppercase tracking-wide text-stone-500">{{ t('admin.settings.features.affiliate.inviteeRebates.col.lastRecharge') }}</th>
                      </tr>
                    </thead>
                    <tbody class="divide-y divide-stone-100 bg-white/60 dark:divide-dark-800 dark:bg-dark-900/20">
                      <tr v-if="affiliateInviteeRebateState.loading">
                        <td colspan="6" class="px-3 py-6 text-center text-sm text-gray-500">
                          {{ t('common.loading') }}
                        </td>
                      </tr>
                      <tr v-else-if="affiliateInviteeRebateState.entries.length === 0">
                        <td colspan="6" class="px-3 py-6 text-center text-sm text-gray-500">
                          {{ t('admin.settings.features.affiliate.inviteeRebates.empty') }}
                        </td>
                      </tr>
                      <tr
                        v-for="entry in affiliateInviteeRebateState.entries"
                        :key="`${entry.inviter_id}-${entry.invitee_id}`"
                        class="transition-colors hover:bg-emerald-50/40 dark:hover:bg-dark-800/40"
                      >
                        <td class="px-3 py-2 text-sm">
                          <div class="font-medium text-stone-950 dark:text-white">{{ entry.inviter_email || `#${entry.inviter_id}` }}</div>
                          <div class="mt-0.5 font-mono text-xs text-stone-500">{{ entry.inviter_aff_code || '-' }}</div>
                        </td>
                        <td class="px-3 py-2 text-sm">
                          <div class="font-medium text-stone-950 dark:text-white">{{ entry.invitee_email || `#${entry.invitee_id}` }}</div>
                          <div class="mt-0.5 text-xs text-stone-500">{{ entry.invitee_username || formatAffiliateDate(entry.invitee_created_at) }}</div>
                        </td>
                        <td class="px-3 py-2 text-right font-mono text-sm font-medium text-stone-950 dark:text-white">
                          {{ formatAffiliateMoney(entry.total_recharged_amount) }}
                        </td>
                        <td class="px-3 py-2 text-right font-mono text-sm text-stone-600 dark:text-gray-300">
                          {{ formatAffiliateMoney(entry.total_paid_amount) }}
                        </td>
                        <td class="px-3 py-2 text-right font-mono text-sm font-semibold text-emerald-700 dark:text-emerald-300">
                          {{ formatAffiliateMoney(entry.total_rebate_amount) }}
                        </td>
                        <td class="px-3 py-2 text-sm text-stone-600 dark:text-gray-300">
                          {{ formatAffiliateDate(entry.last_recharge_at) }}
                        </td>
                      </tr>
                    </tbody>
                  </table>
                ...
                <div v-if="affiliateInviteeRebateState.total > affiliateInviteeRebateState.pageSize" class="mt-3 flex items-center justify-between text-sm">
                  <span class="text-gray-500">
                    {{ t('admin.settings.features.affiliate.inviteeRebates.totalLabel', { total: affiliateInviteeRebateState.total }) }}
                  </span>
                  <div class="flex items-center gap-2">
                    <button
                      type="button"
                      class="btn btn-secondary btn-sm"
                      :disabled="affiliateInviteeRebateState.page <= 1"
                      @click="changeAffiliateInviteeRebatePage(affiliateInviteeRebateState.page - 1)"
                    >
                      {{ t('pagination.previous') }}
                    </button>
                    <span class="text-gray-500">{{ affiliateInviteeRebateState.page }} / {{ Math.max(1, Math.ceil(affiliateInviteeRebateState.total / affiliateInviteeRebateState.pageSize)) }}</span>
                    <button
                      type="button"
                      class="btn btn-secondary btn-sm"
                      :disabled="affiliateInviteeRebateState.page >= Math.ceil(affiliateInviteeRebateState.total / affiliateInviteeRebateState.pageSize)"
                      @click="changeAffiliateInviteeRebatePage(affiliateInviteeRebateState.page + 1)"
                    >
                      {{ t('pagination.next') }}
                    </button>
        ...
        <div v-show="activeTab === 'payment'" class="space-y-6">
          ...
          <div class="card">
            ...
            <div class="space-y-4 p-6">
              ...
              <template v-if="form.payment_enabled">
                ...
                <!-- Row 5: User-facing payment method routing -->
                <div
                  class="rounded-xl border border-gray-200 bg-gray-50/70 p-4 dark:border-dark-700 dark:bg-dark-800/50"
                >
                  <div class="mb-3">
                    <p class="text-sm font-medium text-gray-900 dark:text-white">
                      {{ localText("前台支付入口", "User-facing payment methods") }}
                    </p>
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                      {{ localText("这里决定用户充值页看到的支付宝/微信按钮实际走哪个服务商。你当前接的是易支付，所以这里要同步到易支付来源。", "Choose which provider backs the Alipay/WeChat buttons shown to users.") }}
                    </p>
                  </div>
                  <div class="grid gap-3 md:grid-cols-2">
                    <div class="rounded-lg border border-gray-200 bg-white p-3 dark:border-dark-700 dark:bg-dark-900/60">
                      <div class="mb-3 flex items-center justify-between">
                        <div>
                          <p class="text-sm font-medium text-gray-900 dark:text-white">
                          ...
                          <p class="text-xs text-gray-500 dark:text-gray-400">
                            {{ localText("展示支付宝按钮，并指定实际支付来源", "Show Alipay and choose the actual source") }}
                          </p>
                        </div>
                        <Toggle v-model="form.payment_visible_method_alipay_enabled" />
                      </div>
                      <Select
                        v-model="form.payment_visible_method_alipay_source"
                        :options="paymentVisibleSourceOptions('alipay')"
                    ...
                    <div class="rounded-lg border border-gray-200 bg-white p-3 dark:border-dark-700 dark:bg-dark-900/60">
                      <div class="mb-3 flex items-center justify-between">
                        <div>
                          ...
                          <p class="text-xs text-gray-500 dark:text-gray-400">
                            {{ localText("展示微信支付按钮，并指定实际支付来源", "Show WeChat Pay and choose the actual source") }}
                          </p>
                        </div>
                        <Toggle v-model="form.payment_visible_method_wxpay_enabled" />
                      </div>
                      <Select
                        v-model="form.payment_visible_method_wxpay_source"
                        :options="paymentVisibleSourceOptions('wxpay')"
          ...
          <div v-if="form.payment_enabled" class="card">
              ...
              <div class="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <h2 class="text-base font-semibold text-gray-900 dark:text-white">
                    {{ localText("订阅套餐管理", "Subscription plans") }}
                  </h2>
                  <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                    {{
                      localText(
                        "这里控制用户充值页能购买的订阅套餐。先创建订阅型分组，再把套餐绑定到分组。",
                        "Configure purchasable subscription plans shown on the payment page.",
                      )
                    }}
                  </p>
                </div>
            ...
            <div class="space-y-4 p-4">
              <div
                v-if="subscriptionGroups.length === 0"
              ...
              >
                {{ localText("还没有可用于售卖的订阅型分组。先去分组管理创建 subscription 类型分组。", "No active subscription groups are available.") }}
              </div>
              ...
              >
                <div class="mb-4 flex items-center justify-between">
                  <h3 class="text-sm font-semibold text-gray-900 dark:text-white">
                    {{ editingPlanId ? localText("编辑套餐", "Edit plan") : localText("新增套餐", "New plan") }}
                  </h3>
                  <button type="button" class="text-sm text-gray-500 hover:text-gray-700 dark:hover:text-gray-300" @click="closePlanEditor">
                    {{ t("common.cancel") }}
                  </button>
                </div>
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                  <div>
                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">
                      {{ localText("套餐名称", "Plan name") }}
                    </label>
                    <input v-model.trim="planForm.name" type="text" class="input" placeholder="Codex Pro" />
                  </div>
                  <div>
                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">
                      {{ localText("绑定分组", "Group") }}
                    </label>
                    <Select
                      v-model="planForm.group_id"
                      :options="subscriptionGroupPlanOptions"
                      :placeholder="localText('选择订阅分组', 'Select group')"
                      searchable
                    />
                  </div>
                  <div>
                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">
                      {{ localText("售价 CNY", "Price CNY") }}
                    </label>
                    <input v-model.number="planForm.price" type="number" min="0.01" step="0.01" class="input" />
                  </div>
                  <div>
                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">
                      {{ localText("划线价，可空", "Original price") }}
                    </label>
                    <input v-model.number="planForm.original_price" type="number" min="0" step="0.01" class="input" />
                  </div>
                  <div>
                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">
                      {{ localText("有效期", "Validity") }}
                    </label>
                    <div class="grid grid-cols-[1fr_120px] gap-2">
                      <input v-model.number="planForm.validity_days" type="number" min="1" step="1" class="input" />
                      <select v-model="planForm.validity_unit" class="input">
                        <option value="day">{{ localText("天", "Days") }}</option>
                        <option value="week">{{ localText("周", "Weeks") }}</option>
                        <option value="month">{{ localText("月", "Months") }}</option>
                        <option value="year">{{ localText("年", "Years") }}</option>
                      </select>
                    </div>
                  </div>
                  <div>
                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">
                      {{ localText("排序", "Sort order") }}
                    </label>
                    <input v-model.number="planForm.sort_order" type="number" step="1" class="input" />
                  </div>
                  <div class="md:col-span-2">
                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">
                      {{ localText("描述", "Description") }}
                    </label>
                    <input v-model.trim="planForm.description" type="text" class="input" :placeholder="localText('适合稳定使用 Codex 的个人或团队', 'Plan description')" />
                  </div>
                  <div class="md:col-span-2">
                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">
                      {{ localText("权益说明，每行一条", "Features, one per line") }}
                    </label>
                    <textarea
                      v-model="planForm.features"
                      class="input min-h-[96px]"
                      :placeholder="localText('Codex 模型接入\\n调用日志与用量统计\\n有效期内按分组额度使用', 'Feature list')"
                    ></textarea>
                  </div>
                </div>
                ...
                <div class="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-gray-200 pt-4 dark:border-dark-700">
                  <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                    <input v-model="planForm.for_sale" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500" />
                    {{ localText("前台可购买", "For sale") }}
                  </label>
                  <div class="flex items-center gap-2">
                    <button type="button" class="btn btn-secondary btn-sm" @click="closePlanEditor">
                      {{ t("common.cancel") }}
                    </button>
                    <button type="button" class="btn btn-primary btn-sm" :disabled="planSaving" @click="savePlan">
                      {{ planSaving ? t("admin.settings.saving") : localText("保存套餐", "Save plan") }}
                    </button>
                  </div>
                </div>
              </div>

              <div v-if="plansLoading && plans.length === 0" class="flex items-center justify-center py-6">
                <div class="h-5 w-5 animate-spin rounded-full border-2 border-primary-500 border-t-transparent" />
              </div>

              <div v-else-if="plans.length === 0" class="rounded-lg border border-dashed border-gray-200 px-4 py-8 text-center text-sm text-gray-500 dark:border-dark-700 dark:text-gray-400">
                {{ localText("还没有订阅套餐。新增套餐后，用户充值页会出现“订阅”页签。", "No subscription plans yet.") }}
              </div>
              ...
              <div v-else class="overflow-hidden rounded-lg border border-gray-200 dark:border-dark-700">
                <table class="min-w-full divide-y divide-gray-200 text-sm dark:divide-dark-700">
                  <thead class="bg-gray-50 text-left text-xs font-medium uppercase tracking-wide text-gray-500 dark:bg-dark-800 dark:text-gray-400">
                    <tr>
                      <th class="px-4 py-3">{{ localText("套餐", "Plan") }}</th>
                      <th class="px-4 py-3">{{ localText("分组", "Group") }}</th>
                      <th class="px-4 py-3">{{ localText("价格", "Price") }}</th>
                      <th class="px-4 py-3">{{ localText("有效期", "Validity") }}</th>
                      <th class="px-4 py-3">{{ localText("状态", "Status") }}</th>
                      <th class="px-4 py-3 text-right">{{ localText("操作", "Actions") }}</th>
                    </tr>
                  </thead>
                  <tbody class="divide-y divide-gray-100 bg-white dark:divide-dark-700 dark:bg-dark-800/40">
                    <tr v-for="plan in plans" :key="plan.id">
                      <td class="px-4 py-3">
                        <div class="font-medium text-gray-900 dark:text-white">{{ plan.name }}</div>
                        <div v-if="plan.description" class="mt-0.5 max-w-md truncate text-xs text-gray-500 dark:text-gray-400">{{ plan.description }}</div>
                      </td>
                      <td class="px-4 py-3 text-gray-600 dark:text-gray-300">
                        {{ planGroupName(plan.group_id) }}
                      </td>
                      <td class="px-4 py-3 text-gray-900 dark:text-white">
                        ¥{{ Number(plan.price || 0).toFixed(2) }}
                      </td>
                      <td class="px-4 py-3 text-gray-600 dark:text-gray-300">
                        {{ formatPlanValidity(plan.validity_days, plan.validity_unit) }}
                      </td>
                      <td class="px-4 py-3">
                        <span
                          :class="[
                        ...
                        >
                          {{ plan.for_sale ? localText("可购买", "For sale") : localText("已下架", "Hidden") }}
                        </span>
                      </td>
                      <td class="px-4 py-3">
                        <div class="flex justify-end gap-2">
                          <button type="button" class="btn btn-secondary btn-sm" @click="openEditPlan(plan)">
                            {{ t("common.edit") }}
                          </button>
                          <button type="button" class="btn btn-secondary btn-sm text-red-600 hover:text-red-700 dark:text-red-400" @click="confirmDeletePlan(plan)">
                            {{ t("common.delete") }}
                          </button>
                        </div>
                      </td>
        ...
        <div v-show="activeTab === 'email'" class="space-y-6">
          ...
          <!-- SMTP Settings - required by email verification and password reset -->
          <div v-if="form.email_verify_enabled || form.password_reset_enabled" class="card">
            <div
          ...
          <!-- Send Test Email - required by email verification and password reset -->
          <div v-if="form.email_verify_enabled || form.password_reset_enabled" class="card">
            <div
              class="border-b border-gray-100 px-6 py-4 dark:border-dark-700"
            >
      ...
      <ConfirmDialog
        ...
        :title="localText('删除订阅套餐', 'Delete subscription plan')"
        :message="localText('确认删除这个订阅套餐？有未完成订单时后端会拒绝删除。', 'Delete this subscription plan?')"
        :confirm-text="t('common.delete')"
        danger
...
import { affiliatesAPI, type AffiliateAdminEntry, type AffiliateInviteeRebateEntry, type SimpleUser as AffiliateSimpleUser } from "@/api/admin/affiliates";
...
function paymentVisibleSourceOptions(method: PaymentVisibleMethod) {
  return [
    { value: "", label: localText("未选择", "Not selected") },
...
const allPaymentTypes = computed(() => [
  ...
  { value: "usdc", label: t("payment.methods.usdc") },
...
const planForm = reactive<PlanFormState>(createEmptyPlanForm());

const providerKeyOptions = computed(() => [
  { value: "easypay", label: t("admin.settings.payment.providerEasypay") },
  { value: "alipay", label: t("admin.settings.payment.providerAlipay") },
  { value: "wxpay", label: t("admin.settings.payment.providerWxpay") },
  { value: "usdc", label: t("admin.settings.payment.providerUsdc") },
...
const subscriptionGroupPlanOptions = computed(() =>
  subscriptionGroups.value.map((group) => ({
    value: group.id,
...
function createEmptyPlanForm(): PlanFormState {
  return {
    group_id: Number(subscriptionGroups.value[0]?.id || 0),
    name: "",
    description: "",
    price: 0,
    original_price: null,
...
function resetPlanForm(next: PlanFormState) {
  Object.assign(planForm, next);
}

function readPlanFeatures(plan: SubscriptionPlan): string {
  const raw = (plan as unknown as { features?: unknown }).features;
...
function planGroupName(groupID: number): string {
  const group = subscriptionGroups.value.find((item) => Number(item.id) === Number(groupID));
  return group ? `${group.name} #${group.id}` : `#${groupID}`;
}

function formatPlanValidity(days: number, unit: string): string {
...
function openCreatePlan() {
  ...
  resetPlanForm(createEmptyPlanForm());
  showPlanEditor.value = true;
}

function openEditPlan(plan: SubscriptionPlan) {
  editingPlanId.value = Number(plan.id);
  resetPlanForm({
    group_id: Number(plan.group_id || 0),
    name: plan.name || "",
    description: plan.description || "",
    price: Number(plan.price || 0),
    original_price: plan.original_price == null ? null : Number(plan.original_price),
    validity_days: Number(plan.validity_days || 30),
    validity_unit: plan.validity_unit || "day",
    features: readPlanFeatures(plan),
    product_name: (plan as unknown as { product_name?: string }).product_name || "",
    for_sale: plan.for_sale !== false,
...
function buildPlanPayload(): Record<string, unknown> | null {
  const groupID = Number(planForm.group_id);
  const price = Number(planForm.price);
  const validityDays = Math.floor(Number(planForm.validity_days));

  if (!planForm.name.trim()) {
    appStore.showError(localText("请填写套餐名称", "Plan name is required"));
    return null;
  }
  if (!groupID) {
    appStore.showError(localText("请选择订阅分组", "Please select a group"));
    return null;
  }
  if (!Number.isFinite(price) || price <= 0) {
    appStore.showError(localText("套餐售价必须大于 0", "Plan price must be greater than 0"));
    return null;
  }
  if (!Number.isFinite(validityDays) || validityDays <= 0) {
    appStore.showError(localText("有效期必须大于 0", "Validity must be greater than 0"));
    return null;
  }
  ...
  const payload: Record<string, unknown> = {
    ...
    validity_unit: planForm.validity_unit || "day",
    features: planForm.features,
    product_name: planForm.product_name.trim() || planForm.name.trim(),
    for_sale: planForm.for_sale,
    sort_order: Math.floor(Number(planForm.sort_order) || 0),
  };

  const originalPrice = Number(planForm.original_price);
...
async function savePlan() {
  ...
  try {
    if (editingPlanId.value) {
      await adminAPI.payment.updatePlan(editingPlanId.value, payload);
    } else {
      await adminAPI.payment.createPlan(payload);
    }
    appStore.showSuccess(localText("套餐已保存", "Plan saved"));
    closePlanEditor();
    await loadPlans();
  } catch (err: unknown) {
    appStore.showError(extractI18nErrorMessage(err, t, "payment.errors", t("common.error")));
...
function confirmDeletePlan(plan: SubscriptionPlan) {
  deletingPlanId.value = Number(plan.id);
...
async function handleDeletePlan() {
  ...
  try {
    await adminAPI.payment.deletePlan(deletingPlanId.value);
    appStore.showSuccess(t("common.deleted"));
...
interface AffiliateInviteeRebateState {
  loading: boolean;
  entries: AffiliateInviteeRebateEntry[];
  total: number;
  page: number;
  pageSize: number;
  search: string;
...
const affiliateInviteeRebateState = reactive<AffiliateInviteeRebateState>({
  loading: false,
  entries: [],
  total: 0,
  page: 1,
...
async function handleAffiliateConfirm() {
  ...
  try {
    ...
    await Promise.all([loadAffiliateUsers(), loadAffiliateInviteeRebates()]);
  } catch (err) {
    appStore.showError(extractApiErrorMessage(err, t("common.error")));
  }
}
...
async function loadAffiliateInviteeRebates() {
  affiliateInviteeRebateState.loading = true;
  try {
    const res = await affiliatesAPI.listInviteeRebates({
      page: affiliateInviteeRebateState.page,
      page_size: affiliateInviteeRebateState.pageSize,
      search: affiliateInviteeRebateState.search,
    });
    affiliateInviteeRebateState.entries = res.items ?? [];
    affiliateInviteeRebateState.total = res.total ?? 0;
  } catch (err) {
    appStore.showError(extractApiErrorMessage(err, t("common.error")));
  } finally {
    affiliateInviteeRebateState.loading = false;
  }
}
...
function onAffiliateInviteeRebateSearchInput() {
  debounceTimer(affiliateInviteeRebateState, 300, () => {
    affiliateInviteeRebateState.page = 1;
    loadAffiliateInviteeRebates();
  });
}
...
function changeAffiliateInviteeRebatePage(page: number) {
  if (page < 1) return;
  affiliateInviteeRebateState.page = page;
  loadAffiliateInviteeRebates();
}
...
function formatAffiliateMoney(value: number | null | undefined): string {
  return formatCurrency(value ?? 0, 'CNY');
}

function formatAffiliateDate(value?: string | null): string {
