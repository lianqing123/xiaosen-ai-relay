<template>
  <AppLayout>
    <TablePageLayout>
      <template #filters>
        <div
          v-if="isKiroAccountScope"
          class="mb-3 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-blue-100 bg-blue-50/80 px-4 py-3 text-sm text-blue-900 dark:border-blue-900/40 dark:bg-blue-950/30 dark:text-blue-100"
        >
          <div class="flex flex-col gap-1">
            <span class="font-semibold">Kiro 账号管理</span>
            <span class="text-xs text-blue-700 dark:text-blue-200">原生表格管理 Kiro Gateway 线路账号；官方 token 账号额度在下方独立展示。</span>
          </div>
          <RouterLink to="/admin/channels/kiro" class="btn btn-secondary btn-sm">Kiro 接入配置</RouterLink>
        </div>
        <section
          v-if="isKiroAccountScope"
          class="mb-3 overflow-hidden rounded-lg border border-gray-200 bg-white dark:border-gray-700 dark:bg-dark-800"
        >
          <div class="flex flex-wrap items-center justify-between gap-3 border-b border-gray-100 px-4 py-3 dark:border-gray-700">
            <div class="min-w-0">
              <p class="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Kiro 官方账号池</p>
              <h2 class="mt-1 text-sm font-semibold text-gray-900 dark:text-white">账号额度明细</h2>
              <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ kiroOfficialSummary }}</p>
              <p v-if="kiroOfficialError" class="mt-1 text-xs text-red-600 dark:text-red-300">{{ kiroOfficialError }}</p>
            </div>
            <button type="button" class="btn btn-secondary btn-sm" :disabled="kiroOfficialLoading" @click="loadKiroOfficialStatus">
              <Icon name="refresh" size="sm" :class="{ 'animate-spin': kiroOfficialLoading }" />
              <span>刷新官方账号</span>
            </button>
          </div>
          <div v-if="kiroOfficialLoading && !officialQuotaRows.length" class="px-4 py-5 text-sm text-gray-500 dark:text-gray-400">
            正在同步 Kiro 官方账号...
          </div>
          <div v-else-if="officialQuotaRows.length" class="max-h-80 overflow-auto">
            <table class="min-w-full divide-y divide-gray-100 text-sm dark:divide-gray-700">
              <thead class="sticky top-0 bg-gray-50 text-xs text-gray-500 dark:bg-dark-700 dark:text-gray-300">
                <tr>
                  <th class="px-4 py-2 text-left font-medium">账号</th>
                  <th class="px-4 py-2 text-left font-medium">套餐</th>
                  <th class="px-4 py-2 text-left font-medium">剩余额度</th>
                  <th class="px-4 py-2 text-left font-medium">已用 / 总额</th>
                  <th class="px-4 py-2 text-left font-medium">重置时间</th>
                  <th class="px-4 py-2 text-left font-medium">状态</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                <tr v-for="row in officialQuotaRows" :key="row.key" class="text-gray-700 dark:text-gray-200">
                  <td class="px-4 py-2">
                    <div class="flex min-w-40 flex-col">
                      <span class="font-medium text-gray-900 dark:text-white">{{ row.name }}</span>
                      <span class="max-w-44 truncate text-xs text-gray-500 dark:text-gray-400" :title="row.identity">{{ row.identity }}</span>
                    </div>
                  </td>
                  <td class="px-4 py-2">{{ row.plan }}</td>
                  <td class="px-4 py-2 font-medium">{{ row.remaining }}</td>
                  <td class="px-4 py-2">{{ row.usage }}</td>
                  <td class="px-4 py-2">{{ row.resetAt }}</td>
                  <td class="px-4 py-2">
                    <span :class="['inline-flex rounded-full px-2 py-0.5 text-xs font-medium', row.tone === 'good' ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300' : 'bg-red-50 text-red-700 dark:bg-red-900/30 dark:text-red-300']">
                      {{ row.status }}
                    </span>
                  </td>
                </tr>
          ...
          <div v-else class="px-4 py-5 text-sm text-gray-500 dark:text-gray-400">
            暂未读取到 Kiro 官方账号额度；刷新后仍为空时检查 token 是否过期。
          </div>
        </section>
        <div class="flex flex-wrap-reverse items-start justify-between gap-3">
          <AccountTableFilters
            ...
            :filters="params"
            :groups="groups"
            :hidden-filters="accountFilterHiddenKeys"
          ...
          <AccountTableActions
            :loading="loading"
            :show-sync="!isKiroAccountScope"
            :show-create="!isKiroAccountScope"
          ...
          >
            ...
            <template #beforeCreate>
              <button v-if="!isKiroAccountScope" @click="showImportData = true" class="btn btn-secondary">
                {{ t('admin.accounts.dataImport') }}
              </button>
              <button v-if="!isKiroAccountScope" @click="openExportDataDialog" class="btn btn-secondary">
      ...
      <template #table>
        <AccountBulkActionsBar :selected-ids="selIds" :show-refresh-token="!isKiroAccountScope" @delete="handleBulkDelete" @reset-status="handleBulkResetStatus" @refresh-token="handleBulkRefreshToken" @edit="showBulkEdit = true" @clear="clearSelection" @select-page="selectPage" @toggle-schedulable="handleBulkToggleSchedulable" />
...
import type { KiroGatewayStatus, KiroOfficialQuotaAccount } from '@/api/admin/kiro'

const { t } = useI18n()
const appStore = useAppStore()
const authStore = useAuthStore()
const props = withDefaults(defineProps<{
  accountScope?: 'all' | 'kiro'
}>(), {
  accountScope: 'all'
})
const isKiroAccountScope = computed(() => props.accountScope === 'kiro')
const kiroLockedFilters = {
  platform: 'anthropic',
  type: 'apikey',
  source: 'kiro-gateway'
} as const
const accountFilterHiddenKeys = computed(() => isKiroAccountScope.value ? ['platform', 'type', 'privacy_mode'] : [])
...
const kiroGatewayStatus = ref<KiroGatewayStatus | null>(null)
const kiroOfficialLoading = ref(false)
const kiroOfficialError = ref('')
const kiroOfficialQuota = computed(() => kiroGatewayStatus.value?.official_quota ?? null)
const kiroOfficialVisibleAccounts = computed<KiroOfficialQuotaAccount[]>(() => {
  return (kiroOfficialQuota.value?.accounts ?? []).filter((item) => !item.deduplicated)
})
const kiroOfficialAvailableCount = computed(() => kiroOfficialVisibleAccounts.value.filter((item) => item.available).length)
const officialQuotaRows = computed(() => {
  return kiroOfficialVisibleAccounts.value.map((item, index) => ({
    key: item.user_id || item.credential_sha256 || `kiro-official-${index}`,
    name: `账号 ${String(index + 1).padStart(2, '0')}`,
    identity: item.error || (item.user_id ? `User ${shortKiroId(item.user_id)}` : '已脱敏'),
    plan: item.subscription_title || kiroOfficialQuota.value?.subscription_title || 'Kiro',
    remaining: `${formatKiroCreditAmount(item.remaining)} credits`,
    usage: `${formatKiroCreditAmount(item.current_usage)} / ${formatKiroCreditAmount(item.usage_limit)} credits`,
    resetAt: item.next_reset_at ? formatDateTime(item.next_reset_at) : '-',
    status: item.available ? '正常' : '异常',
    tone: item.available ? 'good' : 'danger'
  }))
})
const kiroOfficialSummary = computed(() => {
  if (kiroOfficialLoading.value && !officialQuotaRows.value.length) return '正在读取 Kiro Gateway 官方账号池'
  if (kiroOfficialError.value && !officialQuotaRows.value.length) return '官方账号池读取失败'
  const total = kiroOfficialQuota.value?.unique_user_count || kiroOfficialQuota.value?.account_count || officialQuotaRows.value.length
  if (!total) return '等待 Kiro 官方账号数据'
  const quota = kiroOfficialQuota.value
  const quotaText = quota && quota.available
    ? `总剩余 ${formatKiroCreditAmount(quota.remaining)} / ${formatKiroCreditAmount(quota.usage_limit)} credits`
    : '额度暂不可用'
  return `${kiroOfficialAvailableCount.value}/${total} 个账号可用，${quotaText}`
})
...
const DEFAULT_HIDDEN_COLUMNS = isKiroAccountScope.value
  ? ['proxy', 'notes']
  : ['today_stats', 'proxy', 'notes', 'priority', 'rate_multiplier']
const HIDDEN_COLUMNS_KEY = isKiroAccountScope.value ? 'kiro-account-hidden-columns' : 'account-hidden-columns'
...
function formatKiroCreditAmount(value: number | null | undefined): string {
...
function shortKiroId(value: string): string {
  const trimmed = value.trim()
  if (trimmed.length <= 8) return trimmed
  return `${trimmed.slice(0, 4)}...${trimmed.slice(-4)}`
}

const loadKiroOfficialStatus = async () => {
  if (!isKiroAccountScope.value) return
  kiroOfficialLoading.value = true
  kiroOfficialError.value = ''
  try {
    kiroGatewayStatus.value = await adminAPI.kiro.getStatus()
    const quotaError = kiroGatewayStatus.value?.official_quota?.error
    if (quotaError) {
      kiroOfficialError.value = quotaError
    }
  } catch (error: any) {
    kiroOfficialError.value = error?.response?.data?.message || error?.message || 'Kiro 官方账号读取失败'
  } finally {
    kiroOfficialLoading.value = false
  }
}
...
const {
  ...
  initialParams: {
    platform: isKiroAccountScope.value ? kiroLockedFilters.platform : '',
    type: isKiroAccountScope.value ? kiroLockedFilters.type : '',
    status: '',
    privacy_mode: '',
    group: '',
    search: '',
    source: isKiroAccountScope.value ? kiroLockedFilters.source : '',
...
const applyAccountScopeLocks = () => {
  if (!isKiroAccountScope.value) return
  Object.assign(params, kiroLockedFilters)
}

const handleFilterUpdate = (newFilters: Record<string, any>) => {
  Object.assign(params, newFilters)
  applyAccountScopeLocks()
}
...
const handleManualRefresh = async () => {
  await load()
  if (isKiroAccountScope.value) {
    await loadKiroOfficialStatus()
  }
...
const accountMatchesSource = (account: Account, source: string): boolean => {
  const extra = account.extra as Record<string, unknown> | undefined
  if (source === kiroLockedFilters.source) {
    return extra?.source === kiroLockedFilters.source || extra?.deployment_path === '/opt/kiro-gateway'
  }
...
const accountMatchesCurrentFilters = (account: Account) => {
  ...
  if (filters.source && !accountMatchesSource(account, filters.source)) return false
  if (filters.status) {
    const now = Date.now()
