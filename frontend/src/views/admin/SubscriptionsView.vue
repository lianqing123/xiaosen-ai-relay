  ...
  <AppLayout>
          ...
          <template #cell-usage="{ row }">
            <div class="min-w-[280px] space-y-2">
              ...
              <button
                ...
                type="button"
                :data-test="`reset-quota-inline-${row.id}`"
                :disabled="resettingQuota && resettingSubscription?.id === row.id"
                class="inline-flex items-center gap-1 rounded-md border border-orange-200 bg-orange-50 px-2.5 py-1 text-xs font-medium text-orange-700 transition hover:border-orange-300 hover:bg-orange-100 disabled:cursor-not-allowed disabled:opacity-50 dark:border-orange-500/30 dark:bg-orange-900/20 dark:text-orange-200 dark:hover:border-orange-400/50 dark:hover:bg-orange-900/30"
      ...
      >
        ...
        <div>
          <div class="mb-2 flex items-center justify-between gap-3">
            <label class="input-label mb-0">{{ t('admin.subscriptions.form.validityDays') }}</label>
            <span class="text-xs text-gray-500 dark:text-gray-400">
              {{ t('admin.subscriptions.maxValidityHint') }}
            </span>
          </div>
          <div class="mb-3 grid grid-cols-4 gap-2">
            <button
              v-for="days in assignValidityPresets"
              :key="days"
              type="button"
              :data-test="`assign-validity-preset-${days}`"
              class="rounded-lg border px-3 py-2 text-sm font-medium transition"
              :class="
                assignForm.validity_days === days
                  ? 'border-primary-500 bg-primary-50 text-primary-700 dark:border-primary-400 dark:bg-primary-900/30 dark:text-primary-200'
                  : 'border-gray-200 bg-white text-gray-700 hover:border-primary-300 hover:text-primary-600 dark:border-dark-600 dark:bg-dark-800 dark:text-gray-300'
              "
              @click="setAssignValidityDays(days)"
            >
              {{ t('admin.subscriptions.dayPreset', { days }) }}
            </button>
          </div>
          <div class="relative">
            <input
              v-model.number="assignForm.validity_days"
              data-test="assign-validity-days-input"
              type="number"
              min="1"
              max="36500"
              class="input pr-12"
            ...
            <span class="pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 text-sm text-gray-400">
              {{ t('admin.subscriptions.daysUnit') }}
            </span>
          </div>
          <p class="input-hint">
            {{ t('admin.subscriptions.validityHint') }}
            <span v-if="assignExpiryPreview">
              · {{ t('admin.subscriptions.expiryPreview', { date: assignExpiryPreview }) }}
            </span>
          </p>
        </div>
      </form>
    ...
    >
      <form
        ...
        id="reset-subscription-quota-form"
        class="space-y-4"
      >
        <div class="rounded-lg bg-gray-50 p-4 dark:bg-dark-700">
          <p class="text-sm text-gray-600 dark:text-gray-400">
        ...
        <div class="grid gap-2">
          <button
            ...
            :key="option.value"
            type="button"
            :data-test="`reset-quota-option-${option.value}`"
            :aria-pressed="resetQuotaScope === option.value"
            class="flex items-start gap-3 rounded-lg border px-4 py-3 text-left transition"
            :class="
              resetQuotaScope === option.value
                ? 'border-orange-400 bg-orange-50 text-orange-900 dark:border-orange-500/70 dark:bg-orange-900/20 dark:text-orange-100'
                : 'border-gray-200 bg-white text-gray-700 hover:border-orange-300 hover:bg-orange-50/60 dark:border-dark-600 dark:bg-dark-800 dark:text-gray-300 dark:hover:border-orange-500/50 dark:hover:bg-orange-900/10'
            "
            :disabled="resettingQuota"
            @click="confirmResetQuota(option.value)"
          >
            <span
              ...
              "
            >
              <span v-if="resetQuotaScope === option.value" class="h-1.5 w-1.5 rounded-full bg-white"></span>
            </span>
            <span>
...
const resetQuotaScope = ref<ResetQuotaScope | null>(null)
const resetQuotaOptions = computed<Array<{ value: ResetQuotaScope; label: string; description: string }>>(() => [
  {
    value: 'daily',
    label: t('admin.subscriptions.resetQuotaScopes.daily'),
    description: t('admin.subscriptions.resetQuotaScopes.dailyDesc')
  },
  {
    value: 'weekly',
    label: t('admin.subscriptions.resetQuotaScopes.weekly'),
    description: t('admin.subscriptions.resetQuotaScopes.weeklyDesc')
  },
  {
    value: 'monthly',
    label: t('admin.subscriptions.resetQuotaScopes.monthly'),
    description: t('admin.subscriptions.resetQuotaScopes.monthlyDesc')
  },
  {
    value: 'all',
    label: t('admin.subscriptions.resetQuotaScopes.all'),
    description: t('admin.subscriptions.resetQuotaScopes.allDesc')
  }
])

const getResetQuotaPayload = (scope: ResetQuotaScope) => ({
...
const assignValidityPresets = [7, 30, 90, 365]

const extendForm = reactive({
  days: 30
})

const setAssignValidityDays = (days: number) => {
  assignForm.validity_days = days
}

const assignExpiryPreview = computed(() => {
  if (!assignForm.validity_days || assignForm.validity_days < 1) return ''
  const expiresAt = new Date()
  expiresAt.setDate(expiresAt.getDate() + assignForm.validity_days)
  return formatDateOnly(expiresAt.toISOString())
})
...
const confirmResetQuota = async (scope: ResetQuotaScope) => {
  if (!resettingSubscription.value) return
  if (resettingQuota.value) return
  resetQuotaScope.value = scope
  resettingQuota.value = true
  try {
    await adminAPI.subscriptions.resetQuota(resettingSubscription.value.id, getResetQuotaPayload(scope))
