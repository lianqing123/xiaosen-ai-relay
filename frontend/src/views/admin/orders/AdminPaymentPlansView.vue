      ...
      <DataTable :columns="planColumns" :data="plans" :loading="plansLoading">
        ...
        <template #cell-group_id="{ value, row }">
          <div class="space-y-1">
            ...
            <div v-if="planExtraGroups(row).length > 0" class="flex max-w-[260px] flex-wrap gap-1">
              <span
                v-for="group in planExtraGroups(row)"
                :key="group.id"
                class="rounded-full bg-gray-100 px-2 py-0.5 text-[11px] font-medium text-gray-600 dark:bg-dark-700 dark:text-gray-300"
...
function normalizedPlanGroupIds(plan: SubscriptionPlan): number[] {
  const out: number[] = []
  ...
  add(plan.group_id)
  ;(plan.included_group_ids || []).forEach(add)
  return out
}

function planExtraGroups(plan: SubscriptionPlan): AdminGroup[] {
  return normalizedPlanGroupIds(plan)
    .filter(id => id !== Number(plan.group_id))
    .map(id => getGroup(id))
...
function formatValidity(days: number, unit: string): string {
  ...
  if (normalized === 'week') return `${days} ${t('payment.admin.weeks')}`
  if (normalized === 'month') return `${days} ${t('payment.admin.months')}`
  if (normalized === 'year') return `${days} ${t('payment.admin.years')}`
  return `${days} ${t('payment.admin.days')}`
}
