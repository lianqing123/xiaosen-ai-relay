<template>
  <BaseDialog :show="show" :title="plan ? t('payment.admin.editPlan') : t('payment.admin.createPlan')" width="wide" @close="emit('close')">
    <form id="plan-form" @submit.prevent="handleSavePlan" class="space-y-4">
      ...
      <div class="rounded-lg border border-gray-200 bg-white p-3 dark:border-dark-600 dark:bg-dark-800/60">
        <label class="input-label">{{ t('payment.admin.includedGroups') }}</label>
        <div class="grid gap-2 sm:grid-cols-2">
          <label
            v-for="option in includedGroupOptions"
            :key="option.id"
            class="flex cursor-pointer items-center gap-2 rounded-lg border border-gray-100 px-3 py-2 text-sm transition-colors hover:bg-gray-50 dark:border-dark-700 dark:hover:bg-dark-700"
            :class="isGroupIncluded(option.id) ? 'bg-primary-50/60 dark:bg-primary-900/20' : ''"
          >
            <input
              type="checkbox"
              class="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500"
              :checked="isGroupIncluded(option.id)"
              :disabled="option.id === planForm.group_id"
              @change="toggleIncludedGroup(option.id)"
            />
            <span class="min-w-0 flex-1 truncate" :class="platformTextClass(option.platform)">
              {{ option.name }}
            </span>
            <span v-if="option.id === planForm.group_id" class="shrink-0 rounded-full bg-gray-100 px-2 py-0.5 text-[11px] text-gray-500 dark:bg-dark-700 dark:text-gray-400">
...
const planForm = reactive({ name: '', group_id: null as number | null, included_group_ids: [] as number[], description: '', price: 0, original_price: 0, validity_days: 30, validity_unit: 'day', sort_order: 0, for_sale: true })
...
const includedGroupOptions = computed(() =>
  props.groups
    .filter(g => g.subscription_type === 'subscription')
    .map(g => ({ id: Number(g.id), name: `${g.name} #${g.id}`, platform: g.platform })),
)
...
function normalizeIncludedGroupIds(primaryGroupId: number | null, groupIds: number[]): number[] {
  const out: number[] = []
  ...
  add(primaryGroupId)
  groupIds.forEach(add)
  return out
}

function isGroupIncluded(groupId: number): boolean {
  return normalizeIncludedGroupIds(planForm.group_id, planForm.included_group_ids).includes(groupId)
}

function toggleIncludedGroup(groupId: number) {
  if (groupId === planForm.group_id) return
  const selected = new Set(normalizeIncludedGroupIds(planForm.group_id, planForm.included_group_ids))
  if (selected.has(groupId)) selected.delete(groupId)
  else selected.add(groupId)
  planForm.included_group_ids = normalizeIncludedGroupIds(planForm.group_id, Array.from(selected))
}
...
watch(() => props.show, (visible) => {
  if (!visible) return
  if (props.plan) {
    Object.assign(planForm, { name: props.plan.name, group_id: props.plan.group_id, included_group_ids: normalizeIncludedGroupIds(props.plan.group_id, props.plan.included_group_ids || []), description: props.plan.description, price: props.plan.price, original_price: props.plan.original_price || 0, validity_days: props.plan.validity_days, validity_unit: props.plan.validity_unit || 'day', sort_order: props.plan.sort_order || 0, for_sale: props.plan.for_sale })
    planFeaturesText.value = (props.plan.features || []).join('\n')
  } else {
    Object.assign(planForm, { name: '', group_id: null, included_group_ids: [], description: '', price: 0, original_price: 0, validity_days: 30, validity_unit: 'day', sort_order: 0, for_sale: true })
    planFeaturesText.value = ''
  }
})

watch(() => planForm.group_id, (groupId) => {
  planForm.included_group_ids = normalizeIncludedGroupIds(groupId, planForm.included_group_ids)
})
...
function buildPlanPayload() {
  ...
  return {
    ...
    included_group_ids: normalizeIncludedGroupIds(planForm.group_id, planForm.included_group_ids),
