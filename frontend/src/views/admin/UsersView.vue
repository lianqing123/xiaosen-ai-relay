<template>
  <AppLayout>
      ...
      <template #filters>
        <div class="flex flex-wrap items-center gap-3">
          ...
          <div class="flex flex-wrap items-center justify-end gap-2">
            ...
            <button
              @click="openInactiveBalanceCleanup"
              class="btn btn-secondary flex-1 border-amber-200 bg-amber-50 text-amber-700 hover:bg-amber-100 dark:border-amber-900/40 dark:bg-amber-900/20 dark:text-amber-300 md:flex-initial"
            >
              清理闲置余额
            </button>
    ...
    <Teleport to="body">
      <div
        v-if="showInactiveCleanupModal"
        class="fixed inset-0 z-[10000] flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm"
        @click.self="closeInactiveBalanceCleanup"
      >
        <div class="w-full max-w-3xl overflow-hidden rounded-2xl bg-white shadow-2xl dark:bg-dark-800">
          <div class="border-b border-gray-100 px-6 py-5 dark:border-dark-700">
            <div class="flex items-start justify-between gap-4">
              <div>
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white">三天未使用余额清空</h3>
                <p class="mt-1 text-sm text-gray-500 dark:text-dark-300">
                  仅处理普通活跃用户，跳过管理员、已删除用户和仍有有效订阅的用户。执行后会写入余额历史。
                </p>
              </div>
              <button
                class="rounded-lg p-2 text-gray-400 transition hover:bg-gray-100 hover:text-gray-600 dark:hover:bg-dark-700"
                @click="closeInactiveBalanceCleanup"
              >
                <Icon name="x" size="sm" />
              </button>
            </div>
          ...
          <div class="space-y-5 px-6 py-5">
            <div class="grid gap-3 sm:grid-cols-3">
              <label class="block">
                <span class="text-xs font-medium text-gray-500 dark:text-dark-300">未使用天数</span>
                <input
                  v-model.number="inactiveCleanupDays"
                  type="number"
                  min="3"
                  max="365"
                  class="input mt-1"
              ...
              <label class="block">
                <span class="text-xs font-medium text-gray-500 dark:text-dark-300">本次最多处理</span>
                <input
                  v-model.number="inactiveCleanupLimit"
                  type="number"
                  min="1"
                  max="1000"
                  class="input mt-1"
              ...
              <div class="flex items-end">
                <button
                  ...
                  :disabled="inactiveCleanupLoading"
                  @click="previewInactiveBalanceCleanup"
                >
                  {{ inactiveCleanupLoading ? '预览中...' : '重新预览' }}
                </button>
              </div>
            </div>

            <div
              v-if="inactiveCleanupPreview"
              class="grid gap-3 sm:grid-cols-3"
            >
              <div class="rounded-xl bg-gray-50 p-4 dark:bg-dark-700/60">
                <div class="text-xs text-gray-500 dark:text-dark-300">命中用户</div>
                <div class="mt-1 text-2xl font-semibold text-gray-900 dark:text-white">{{ inactiveCleanupPreview.count }}</div>
              </div>
              <div class="rounded-xl bg-gray-50 p-4 dark:bg-dark-700/60">
                <div class="text-xs text-gray-500 dark:text-dark-300">可清空余额</div>
                <div class="mt-1 text-2xl font-semibold text-gray-900 dark:text-white">
                  ${{ inactiveCleanupPreview.total_balance.toFixed(2) }}
                </div>
              </div>
              <div class="rounded-xl bg-gray-50 p-4 dark:bg-dark-700/60">
                <div class="text-xs text-gray-500 dark:text-dark-300">判定截止</div>
                <div class="mt-2 text-sm font-medium text-gray-900 dark:text-white">
                  {{ formatDateTime(inactiveCleanupPreview.cutoff_at) }}
                </div>
              </div>
            </div>

            <div class="max-h-72 overflow-auto rounded-xl border border-gray-100 dark:border-dark-700">
              <table class="min-w-full divide-y divide-gray-100 text-sm dark:divide-dark-700">
                <thead class="bg-gray-50 text-left text-xs font-medium text-gray-500 dark:bg-dark-700/70 dark:text-dark-300">
                  <tr>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-dark-700">
                  <tr v-if="!inactiveCleanupPreview || inactiveCleanupPreview.candidates.length === 0">
                    <td colspan="4" class="px-4 py-8 text-center text-gray-400">
                      暂无符合条件的用户
                    </td>
                  </tr>
                  <tr
                    v-for="candidate in inactiveCleanupPreview?.candidates || []"
                    :key="candidate.id"
                    class="text-gray-700 dark:text-dark-100"
                  >
                    <td class="px-4 py-3">
                      <div class="font-medium text-gray-900 dark:text-white">{{ candidate.email }}</div>
                      <div class="text-xs text-gray-400">ID {{ candidate.id }}</div>
                    </td>
                    <td class="px-4 py-3 font-medium">${{ candidate.balance.toFixed(2) }}</td>
                    <td class="px-4 py-3">{{ candidate.last_used_at ? formatDateTime(candidate.last_used_at) : '从未使用' }}</td>
                    <td class="px-4 py-3">{{ formatDateTime(candidate.created_at) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <div class="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800 dark:border-amber-900/40 dark:bg-amber-900/20 dark:text-amber-200">
              执行会把命中用户当前余额置为 0，不会改累计充值。建议先确认预览数量和金额。
            </div>
          </div>

          <div class="flex flex-wrap justify-end gap-3 border-t border-gray-100 px-6 py-4 dark:border-dark-700">
            <button class="btn btn-secondary" @click="closeInactiveBalanceCleanup">取消</button>
            <button
              class="btn bg-red-600 text-white hover:bg-red-700 disabled:cursor-not-allowed disabled:opacity-60"
              :disabled="inactiveCleanupExecuting || !inactiveCleanupPreview || inactiveCleanupPreview.count === 0"
              @click="executeInactiveBalanceCleanup"
            >
              {{ inactiveCleanupExecuting ? '执行中...' : '确认清空' }}
            </button>
          </div>
        </div>
      </div>
...
// Inactive balance cleanup state
const showInactiveCleanupModal = ref(false)
const inactiveCleanupDays = ref(3)
const inactiveCleanupLimit = ref(50)
const inactiveCleanupLoading = ref(false)
const inactiveCleanupExecuting = ref(false)
const inactiveCleanupPreview = ref<InactiveBalanceCleanupPreview | null>(null)
...
const openInactiveBalanceCleanup = () => {
  showInactiveCleanupModal.value = true
  previewInactiveBalanceCleanup()
}

const closeInactiveBalanceCleanup = () => {
  if (inactiveCleanupExecuting.value) return
  showInactiveCleanupModal.value = false
}

const previewInactiveBalanceCleanup = async () => {
  inactiveCleanupLoading.value = true
  try {
    inactiveCleanupPreview.value = await adminAPI.users.previewInactiveBalanceCleanup(
      inactiveCleanupDays.value,
      inactiveCleanupLimit.value
    )
  } catch (error: any) {
    const message = error.response?.data?.message || error.message || '预览闲置余额清理失败'
    appStore.showError(message)
    console.error('Error previewing inactive balance cleanup:', error)
  } finally {
    inactiveCleanupLoading.value = false
  }
}

const executeInactiveBalanceCleanup = async () => {
  if (!inactiveCleanupPreview.value || inactiveCleanupPreview.value.count === 0) return
  inactiveCleanupExecuting.value = true
  try {
    const result = await adminAPI.users.executeInactiveBalanceCleanup({
      days: inactiveCleanupDays.value,
      limit: inactiveCleanupLimit.value,
      confirm: 'CLEAR_INACTIVE_BALANCES'
    })
    appStore.showSuccess(`已清空 ${result.cleared_count} 个用户，合计 $${result.cleared_balance.toFixed(2)}`)
    await loadUsers()
    await previewInactiveBalanceCleanup()
  } catch (error: any) {
    const message = error.response?.data?.message || error.message || '执行闲置余额清理失败'
    appStore.showError(message)
    console.error('Error executing inactive balance cleanup:', error)
  } finally {
    inactiveCleanupExecuting.value = false
  }
}
