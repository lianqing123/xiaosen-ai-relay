      ...
      <template #table>
        ...
        >
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center space-x-2">
              <button
                v-if="row.type === 'invitation' && row.status === 'unused'"
                @click="copyToClipboard(buildRegisterLinkForCode(row.code))"
              ...
              >
                ...
                <span class="text-xs">{{ t('admin.promo.copyRegisterLink') }}</span>
              </button>
              <button
...
import { buildInvitationRegisterLink } from '@/utils/invitationRegisterLink'
...
const buildRegisterLinkForCode = (code: string): string => {
  if (typeof window === 'undefined') {
    return `/register?invitation_code=${encodeURIComponent(code.trim())}`
  }
  return buildInvitationRegisterLink(window.location.origin, code)
}

const generatedCodesText = computed(() => {
  return generatedCodes.value
    .map((code) => (code.type === 'invitation' ? buildRegisterLinkForCode(code.code) : code.code))
    .join('\n')
})
