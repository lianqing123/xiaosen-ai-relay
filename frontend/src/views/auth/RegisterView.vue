...
import { resolveInvitationCodeFromQuery } from '@/utils/invitationRegisterLink'
...
function syncInvitationCodeFromQuery(): string {
  const code = resolveInvitationCodeFromQuery(route.query)
  if (code) {
...
onMounted(async () => {
  ...
  try {
    ...
    if (promoCodeEnabled.value) {
      const promoParam = typeof route.query.promo === 'string' ? route.query.promo.trim() : ''
      if (promoParam) {
...
watch(
  ...
  () => {
    ...
    const code = syncInvitationCodeFromQuery()
    if (code) {
