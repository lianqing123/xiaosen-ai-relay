...
describe('PaymentView subscription renewal', () => {
  beforeEach(() => {
    routeState.path = '/purchase'
    routeState.query = {}
    routerReplace.mockReset().mockResolvedValue(undefined)
  ...
  it('opens the renewal plan selector when clicking the current subscription action on the purchase page', async () => {
    const wrapper = shallowMount(PaymentView, {
      global: {
        stubs: {
    ...
    const renewButton = wrapper.findAll('button').find(button => button.text().includes('续费或升级'))
    expect(renewButton).toBeTruthy()

    await renewButton!.trigger('click')
    await flushPromises()

    expect(wrapper.html()).toContain('payment.selectPlan')
  })
})
