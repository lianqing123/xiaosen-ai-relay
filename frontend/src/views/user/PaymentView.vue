        ...
        <template v-else>
          ...
          <template v-else-if="activeTab === 'subscription'">
            ...
            <template v-if="selectedPlan">
              <div class="overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-sm dark:border-dark-700 dark:bg-dark-900">
                <div class="border-b border-gray-100 p-5 dark:border-dark-700">
                  ...
                  <h2 class="mt-2 text-2xl font-bold text-gray-950 dark:text-white">确认开通 {{ selectedPlan.name }}</h2>
                  <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    支付成功后自动开通；已有同组有效订阅时，会从当前到期时间继续顺延。
                  </p>
                </div>
                <div class="p-5">
                  ...
                  <div class="mt-4 grid grid-cols-2 gap-3">
                    ...
                    <div>
                      <span class="text-xs text-gray-400 dark:text-gray-500">有效天数</span>
                      <div class="text-lg font-semibold text-gray-800 dark:text-gray-200">{{ selectedPlanValidityDays }} 天</div>
                    </div>
              ...
              <div v-if="enabledMethods.length === 0" class="card p-6 text-center text-sm text-gray-500 dark:text-gray-400">
                当前没有可用支付方式，请先在后台启用支付宝、微信或 Stripe。
              </div>
              <div v-if="enabledMethods.length >= 1" class="card p-6">
                <PaymentMethodSelector
            ...
            <template v-else>
                ...
                <div class="grid gap-6 p-6 md:grid-cols-[1.5fr_1fr] md:p-8">
                  <div>
                    ...
                    <p class="mt-3 max-w-2xl text-sm leading-7 text-gray-500 dark:text-gray-400">
                      像 GPT 套餐一样购买固定周期。支付成功后自动开通或顺延有效期；到期前再次购买同组套餐，会从当前到期时间继续累加。
                    </p>
                    <div class="mt-5 grid gap-3 text-sm text-gray-600 dark:text-gray-300 sm:grid-cols-3">
                      <div class="rounded-2xl bg-gray-50 px-4 py-3 dark:bg-dark-800/70">
                        <div class="text-xs text-gray-400 dark:text-gray-500">开通方式</div>
                        <div class="mt-1 font-semibold text-gray-900 dark:text-white">一次购买，按期使用</div>
                      </div>
                      <div class="rounded-2xl bg-gray-50 px-4 py-3 dark:bg-dark-800/70">
                        <div class="text-xs text-gray-400 dark:text-gray-500">生效规则</div>
                        <div class="mt-1 font-semibold text-gray-900 dark:text-white">自动开通 / 自动续期</div>
                      </div>
                      <div class="rounded-2xl bg-gray-50 px-4 py-3 dark:bg-dark-800/70">
                        <div class="text-xs text-gray-400 dark:text-gray-500">计费边界</div>
                        <div class="mt-1 font-semibold text-gray-900 dark:text-white">按分组额度控制</div>
                      </div>
                    </div>
                  </div>

                  <div class="rounded-3xl border border-gray-200 bg-gray-50 p-5 dark:border-dark-700 dark:bg-dark-800/70">
                    <div class="flex items-center justify-between">
                      <span class="text-sm font-semibold text-gray-900 dark:text-white">当前订阅</span>
                      <span
                        :class="[
                      ...
                      >
                        {{ primaryActiveSubscription ? '已开通' : '未开通' }}
                      </span>
                    </div>
                    <div v-if="primaryActiveSubscription" class="mt-5 space-y-4">
                      <div>
                        <div class="flex items-center gap-2">
                          <div :class="['h-2 w-2 rounded-full', platformAccentBarClass(primaryActiveSubscription.group?.platform || '')]" />
                          <h2 class="truncate text-xl font-bold text-gray-950 dark:text-white">
                            {{ primaryActiveSubscription.group?.name || t('payment.groupFallback', { id: primaryActiveSubscription.group_id }) }}
                          </h2>
                        </div>
                        <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                          {{ formatSubscriptionExpiry(primaryActiveSubscription.expires_at) }}
                        </p>
                      </div>
                      <button
                        type="button"
                        class="btn btn-primary w-full"
                        @click="openRenewalForGroup(primaryActiveSubscription.group_id)"
                      >
                        续费或升级
                      </button>
                    </div>
                    <div v-else class="mt-5 space-y-4">
                      <p class="text-sm leading-6 text-gray-500 dark:text-gray-400">
                        选择一个套餐后，支付成功会立即绑定到你的账号。后台套餐未上架时这里不会显示可购买项。
                      </p>
                      <button type="button" class="btn btn-secondary w-full" @click="router.push('/subscriptions')">
                        查看订阅状态
                      </button>
                    </div>
                  </div>
              ...
              <div v-if="errorMessage" class="rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 dark:border-red-900/50 dark:bg-red-950/30 dark:text-red-300">
                {{ errorMessage }}
                <span v-if="errorHintMessage" class="block pt-1 text-xs text-red-500 dark:text-red-400">{{ errorHintMessage }}</span>
              </div>

              <div class="flex flex-wrap items-end justify-between gap-3">
                <div>
                  <h2 class="text-xl font-bold text-gray-950 dark:text-white">选择套餐</h2>
                  <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">价格、周期和额度以后台配置为准；同一分组重复购买会自动顺延。</p>
                </div>
              </div>
...
import { getPlanValidityDays, normalizePlanValidityUnit } from '@/utils/subscriptionPlan'
...
const primaryActiveSubscription = computed(() => activeSubscriptions.value[0] ?? null)
...
function formatSubscriptionExpiry(expiresAt: string | null | undefined): string {
  if (!expiresAt) return '长期有效'
  const days = getDaysRemaining(expiresAt)
  const date = new Date(expiresAt)
  const dateText = Number.isNaN(date.getTime()) ? expiresAt : date.toLocaleDateString()
...
// Keep subscription cards roomy enough for quota and positioning copy.
const planGridClass = computed(() => {
  const n = checkout.value.plans.length
  if (n <= 2) return 'grid grid-cols-1 items-stretch gap-6 md:grid-cols-2'
  return 'grid grid-cols-1 items-stretch gap-6 md:grid-cols-2 xl:grid-cols-3'
})
...
const planValiditySuffix = computed(() => {
  ...
  if (u === 'week') return `${selectedPlan.value.validity_days}${t('payment.weeks')}`
  if (u === 'month') return t('payment.perMonth')
  if (u === 'year') return t('payment.perYear')
  return `${selectedPlan.value.validity_days}${t('payment.days')}`
})

const selectedPlanValidityDays = computed(() => {
  if (!selectedPlan.value) return 0
  return getPlanValidityDays(selectedPlan.value.validity_days, selectedPlan.value.validity_unit)
})
...
function openRenewalForGroup(groupIdInput: unknown): boolean {
  ...
  showRenewalModal.value = false
  renewGroupId.value = null
  errorMessage.value = '当前订阅分组暂无可续费套餐，请联系管理员确认套餐是否上架。'
  return false
}

function handleSubscriptionRouteQuery(): void {
  if (route.query.tab !== 'subscription') return
  activeTab.value = 'subscription'
  if (route.query.group) {
    openRenewalForGroup(route.query.group)
  }
}
...
onMounted(async () => {
  try {
    ...
    } else if (checkout.value.balance_disabled || checkout.value.plans.length > 0) {
      activeTab.value = 'subscription'
    }
    // Handle renewal navigation: ?tab=subscription&group=123
    handleSubscriptionRouteQuery()
...
watch(() => [route.query.tab, route.query.group] as const, () => {
