...
export interface InactiveBalanceCleanupCandidate {
  id: number
  email: string
  balance: number
  created_at: string
...
export interface InactiveBalanceCleanupPreview {
  days: number
  cutoff_at: string
  count: number
...
export interface InactiveBalanceCleanupResult {
  days: number
  cutoff_at: string
  cleared_count: number
  cleared_balance: number
  records: InactiveBalanceCleanupCandidate[]
}
...
export async function previewInactiveBalanceCleanup(
  days: number = 3,
  limit: number = 50
): Promise<InactiveBalanceCleanupPreview> {
  const { data } = await apiClient.get<InactiveBalanceCleanupPreview>(
    '/admin/users/inactive-balance-cleanup/preview',
    { params: { days, limit } }
  )
  return data
}

export async function executeInactiveBalanceCleanup(input: {
  days?: number
  limit?: number
  confirm: 'CLEAR_INACTIVE_BALANCES'
}): Promise<InactiveBalanceCleanupResult> {
  const idempotencyKey =
    typeof crypto !== 'undefined' && 'randomUUID' in crypto
      ? crypto.randomUUID()
      : `inactive-balance-cleanup-${Date.now()}-${Math.random().toString(36).slice(2)}`

  const { data } = await apiClient.post<InactiveBalanceCleanupResult>(
    '/admin/users/inactive-balance-cleanup/execute',
    input,
