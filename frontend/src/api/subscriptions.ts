...
function isStandaloneClaudeSubscription(subscription: UserSubscription): boolean {
...
function filterUserVisibleSubscriptions(subscriptions: UserSubscription[]): UserSubscription[] {
  return subscriptions.filter((subscription) => !isStandaloneClaudeSubscription(subscription))
}
...
export async function getMySubscriptions(): Promise<UserSubscription[]> {
  ...
  return filterUserVisibleSubscriptions(response.data)
}
